...
import { FafSchema, validateSchema } from '../schema/faf-schema';
...
export async function validateFafFile(file?: string, options: ValidateOptions = {}) {
  try {
    ...
    if (validation.valid) {
      ...
      if (options.verbose) {
        ...
        console.log(chalk.gray(`   Schema Version: ${validation.schemaVersion}`));
        console.log(chalk.gray(`   Format Version: ${fafData.faf_version || 'unknown'}`));
        console.log(chalk.gray(`   Total Sections: ${validation.sectionsFound}`));
        console.log(chalk.gray(`   Required Fields: ${validation.requiredFieldsFound}/${validation.requiredFieldsTotal}`));
      }
    } else {
