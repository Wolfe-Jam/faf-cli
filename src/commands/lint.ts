...
export async function lintFafFile(file?: string, options: LintOptions = {}) {
  try {
    ...
    try {
      ...
      console.log(chalk.red(parseError instanceof Error ? parseError.message : String(parseError)));
      process.exit(1);
    }
...
function performFormatLinting(fafData: any, content: string, issues: LintIssue[]): void {
