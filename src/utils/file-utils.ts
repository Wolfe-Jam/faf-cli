 ...
 */
export async function findPyprojectToml(startDir: string = process.cwd()): Promise<string | null> {
  let currentDir = startDir;
 ...
 */
export async function findRequirementsTxt(startDir: string = process.cwd()): Promise<string | null> {
  let currentDir = startDir;
 ...
 */
export async function findTsConfig(startDir: string = process.cwd()): Promise<string | null> {
  let currentDir = startDir;
 ...
 */
export async function analyzeTsConfig(filePath: string): Promise<TypeScriptContext | null> {
  try {
...
export async function detectProjectType(projectDir: string = process.cwd()): Promise<string> {
  ...
  if (pythonType !== 'generic') return pythonType;
  
  // TypeScript detection - tsconfig.json presence indicates TypeScript project
  ...
  if (packageJsonPath) {
    try {
      ...
      if (deps.typescript || deps['@types/node'] || Object.keys(deps).some(dep => dep.startsWith('@types/'))) {
        hasTypeScript = true;
      }
  ...
  const files = await glob('**/*.{svelte,jsx,tsx,vue,ts,js,py}', { 
    cwd: projectDir, 
    ignore: ['node_modules/**', 'dist/**', 'build/**', '__pycache__/**', '.venv/**'] 
  });
  ...
  if (files.some(f => f.endsWith('.svelte'))) return hasTypeScript ? 'svelte-ts' : 'svelte';
  if (files.some(f => f.endsWith('.jsx') || f.endsWith('.tsx'))) return hasTypeScript ? 'react-ts' : 'react';
  if (files.some(f => f.endsWith('.vue'))) return hasTypeScript ? 'vue-ts' : 'vue';
 ...
 */
export async function detectPythonProjectType(projectDir: string): Promise<string> {
 ...
 */
async function analyzeRequirementsTxt(filePath: string): Promise<string | null> {
  try {
    const content = await fs.readFile(filePath, 'utf-8');
 ...
 */
export async function detectPythonPatterns(projectDir: string, pythonFiles: string[]): Promise<string> {
  try {
    // Check main Python files first (main.py, app.py, api.py)
    const mainFiles = pythonFiles.filter(f => 
    ...
    for (const file of filesToCheck) {
      ...
      try {
        ...
        // Django patterns  
        if (content.includes('from django.') || content.includes('django.http')) {
          return 'python-django';
        }
        
        // Flask patterns
        if (content.includes('from flask import') || content.includes('Flask(')) {
          return 'python-flask';
        }
        
        // Starlette patterns
        if (content.includes('from starlette.') || content.includes('Starlette(')) {
 ...
 */
function calculateStrictnessLevel(compilerOptions: any): 'basic' | 'strict' | 'ultra_strict' | 'f1_inspired' {
...
function detectFrameworkIntegration(compilerOptions: any, config: any): string {
  ...
  // Pure TypeScript project detection
  if (compilerOptions.target && compilerOptions.module && !compilerOptions.jsx) {
 ...
 */
function assessEngineeringQuality(compilerOptions: any): 'standard' | 'professional' | 'f1_inspired' {
  let qualityScore = 0;
