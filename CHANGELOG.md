...
- **A failed write no longer leaves a half-written file.** The writers for `.faf`, CLAUDE.md, AGENTS.md, GEMINI.md, `.cursorrules`, the Copilot file, MEMORY.md, `llms.txt`, `soul.fafm`, `.faf-dna`, `project.html`, the Server Card, `.grok/config.toml` and the `faf cards` output write a temp file next to the target, flush it to disk and rename it over the original in one step, keeping the original's permissions. On a full disk, a quota or a killed process the original is left exactly as it was, and the error says "not written; original kept". Before, the file was cut short and the user's text after the cut was gone. A read-only file is not replaced.
...
- **faf no longer rewrites a `.faf-dna` it cannot prove it wrote.** `faf auto` and `faf refresh` rewrote any `.faf-dna` in faf's shape, so a 20-digit number was rounded, keys that JSON reorders were reordered, a repeated key lost its first value, `\u` escapes were decoded, and a note on the peak milestone went with it. Growth is now added only when the file's text is exactly what faf itself writes for it (2-space JSON and a final newline) and no key of yours sits in an entry faf replaces. Any other `.faf-dna` is read, left byte for byte and gets no growth, and `faf auto`, `faf refresh` and `faf dna` say why in one line.
