...
### Changed
- **One-time step for an existing project.html, Server Card, A2A card, `faf server-card --out` file or `faf taf --output` snapshot.** faf now replaces one of these whole files only when it is exactly what faf last wrote. The first run that would change one written before 7.13 refuses it in one line, and that run exits 1. Check the refused file for hand edits you want to keep, then run the same command once with `--force`, which writes faf's render over the whole file, text and all; after that faf recognises its own output.

  Each of these files now carries a hash of faf's own render: project.html a `<meta name="faf-render" content="sha256:…">` line (the hash of the page without that line), the JSON files `_meta["one.faf/render"]` (the hash of the file without that key). The MCP Server Card and registry server.json schemas leave `_meta` open for namespaced keys (the registry keeps only its publisher-provided key on publish). The A2A Agent Card has no `_meta` field, and the A2A spec says a reader should ignore a field it does not know, as the official Python and Go SDKs do. On each write faf recomputes the hash:
  - it matches: the file is faf's, byte for byte, and faf replaces it;
  - it does not match: the file was edited since, and faf leaves it: "<file> was edited since faf wrote it — faf left it unchanged. Use --force to replace it." A project.html line that starts like faf's render line but is not its exact form (a trailing space, an upper-case hash) counts as edited too;
  - the file has faf's mark but no hash (written before 7.13): when it is exactly faf's render of the current project.faf, faf leaves it as it is, so a matching project.html stays untouched; otherwise faf refuses it once: "<file> has no faf render hash (written before 7.13), so faf cannot tell whether it was edited — faf left it unchanged. Use --force once to replace it; after that faf recognises its own output." A Server Card, A2A card or `faf taf --output` snapshot from before 7.13 is always refused once: it records the time faf wrote it, and a new render records the time of the new run (a card whose time project.faf sets itself can match);
  - the file has no faf mark at all: "<file> has no <mark>, so faf did not write it — faf left it unchanged. Use --force to replace it.";
  - every line of the file ends CRLF, as a git checkout with `core.autocrlf` leaves it, and that is its only difference: the file is still faf's. faf checks the hash with the CRLF line ends read as LF, and writes the new render with CRLF.

  A `faf export` or `faf cards` run that refuses one file still writes the others, prints one line per refused file and exits 1. In the library, `writeProjectHtml` and `writeServerCard` throw a `SafePathError` with reason `not-owned` in these cases and change nothing; pass `{ force: true }` to replace the file on purpose. A `.fafb` is a compiled binary: faf replaces it only when it starts with the FAFB header.
- **Words typed into a slot count as an empty slot.** `None`, `N/A`, `not applicable`, `unknown`, `null` typed as a string (`"null"`), and the template phrases `describe your project goal`, `development teams` and `cloud platform` — in any case, with any spaces around them — score 0 until the slot is filled. The app-type decides which slots count — `faf auto` marks the slots the app-type leaves out `slotignored`, and the score counts every slot that isn't. `faf auto` handles typed words by slot:
  - a tech slot (every slot but the 6Ws) that the app-type uses: a repo fact replaces the words, comment kept; with no fact the words stay exactly as typed. faf never writes `""` or `slotignored` over them, as 7.12 did (`slotignored` there lifted the score);
  - a tech slot the app-type leaves out (`database` in a library, say): `faf auto` writes `slotignored` (shown as N/A) in place of the words, comment kept, so such a project can reach 100. A real value there is kept. When detection finds no classifying signal, it falls back to `type: library` and notes that on the line (`type: library # found: no classifying signals — fallback`). That fallback is not a fact, so while the note is there `faf auto` writes no `slotignored` over typed words; with a type you set, or one detection found from the repo, it does;
  - a 6W (`human_context.*`): the words are yours. `faf auto` never replaces them; `faf go` asks, shows your words, and Enter keeps them.

  `faf score` prints a line for each slot that holds typed words and scores empty: "stack.hosting says 'unknown' — this app-type needs it, so it counts as empty until filled." or "stack.database says 'None' — this app-type doesn't use it; faf auto marks it slotignored (N/A)." `faf auto` prints the first kind for the slots it could not fill. Nothing is written for a hint, and `faf score --verbose` shows such a slot as empty, never as N/A.
- **Detection fills a slot only from a repo fact.** `faf init`, `faf auto`, `faf git` and the library's detection used to write these values though nothing in the repo says them; now they leave the slot empty:
  - the package manager: no `npm` for a package.json with no lockfile. package.json's `packageManager` field (`"pnpm@9.1.0"`) is read as a fact before any lockfile;
  - the language: no `Unknown` in `main_language` when nothing in the repo names one;
  - Svelte: `Runes` only when package.json names Svelte 5 or later, and `Vite` as the build tool only for SvelteKit or when the repo names Vite;
  - commands: `cargo clippy` only when the repo configures Clippy (`clippy.toml`, or `[lints.clippy]` in Cargo.toml), `zig build test` only when build.zig defines a test step, `pytest` only when the repo names pytest (pytest.ini, conftest.py, or pytest in pyproject.toml, setup.cfg or tox.ini);
  - hosting: no `Docker` from a Dockerfile alone. A Dockerfile shows a container build, not where the app runs; a platform's own config (`vercel.json`, `netlify.toml`, `wrangler.toml`, `fly.toml`, `render.yaml`), a compose file or a SvelteKit adapter still sets hosting;
  - the format finder: no generic words (`Required`, `Containerized`, `Multi-container`, `Orchestrated`, `Cloud`, `API Server`, `GraphQL Server`), no language or tool in a slot it does not fill (backend `Python` from a pyproject.toml, frontend `Zig`, backend `Helm`, frontend `DVC` or `MLflow`, a test runner or a repo bot as CI/CD), no `runtime: Node.js` from a framework config alone (package.json is that fact), and no `Swift App` framework for an Xcode app.

  A project.faf that faf fills now can score lower than before where it used to take one of these values; `faf go` asks for what the repo does not say.
- **`writeFaf` merges into an existing file by default (library users).** `writeFaf(path, data)` on a project.faf that exists now compares `data` with what the file holds and writes only the values that changed. Keys the file has and `data` leaves out are kept, comments and formatting stay, a file that is not valid YAML is refused, and a change that alters nothing writes nothing. Pass `{ replace: true }` for the 7.12 behaviour, a fresh file from `data`; `faf init --force` and `faf git --force` do. `writeFaf` returns a boolean — `false` means nothing was written — so a stub typed as `typeof writeFaf` must return one. `writeFaf` and `updateFafFile` never replace an alias: a change at an alias path is not written, the alias stays, and the rest of the change is written (see `onAliasKept` in Added).
- **A file without faf's block markers is never taken over.** A CLAUDE.md, AGENTS.md, GEMINI.md, `.cursorrules`, Copilot file or `llms.txt` that started with faf's old two-line stamp (`<!-- faf: … -->`) but had no block markers was replaced whole, text and all. faf now replaces only what sits between its own markers, and a file that starts with a BOM keeps it at byte 0. When faf cannot prove a block is its own, its block goes on top and every byte of the file stays below it. That covers:
  - a file with no block markers, whatever its first or last line says — faf's old stamp included;
  - a block below a fence opened in a list item (`- ```bash`) that closes at the item's column, two spaces in: CommonMark closes the fence there, a column-0 reading does not, so faf cannot prove the block below is its own;
  - a marker line with a space or a tab after it: faf never writes one, so it is text;
  - a file that nests more than 100 lists, list items or block quotes before faf's block: faf stops reading there.

  The old text stays below the new block — delete it by hand if you no longer want it. For the old stamp, a block the two readings disagree on and a block past deep nesting, `faf sync` and `faf export` say so in one line. tri-sync writes Claude Code's MEMORY.md by the same rules (see Fixed).
- **A deeply nested `.faf` value never hides faf's block from its next run.** In faf's own block, a value that nests more than 16 block quotes, lists or list items (a 101-deep `>>>…` line in project.goal, or a list nested 52 levels) gets, on each line that would go past 16, one backslash before the marker that would open the 17th (`\>`, `\-`, `1\.`), so faf's next run still finds its block. A block faf still cannot place where its next run finds it again is refused in one line — "<file>: faf could not place its block where the next run finds it again — faf left it unchanged" — and the file stays byte for byte; `faf export` writes its other files and exits 1. `injectFafBlock` and `writeClaudeMemory` throw a `SafePathError` with reason `unplaceable`.
- **A new soul never writes over a soul.fafm that is there.** `faf memory convert` replaced an existing soul.fafm; it now refuses when the output exists — "soul.fafm already exists. Use --force to overwrite." — as `faf init` does. In the library, `FafmSoul.save` of a soul made in memory, or saved to a path it was not loaded from, refuses a file already at the path: "<file> already exists — not written; pass { replace: true } (faf memory convert --force) to replace it". Pass `{ replace: true }` to write over it.
- **`faf hooks --install` no longer creates a missing hooks folder.** faf does not create folders inside `.git`; it says to create `.git/hooks` and run it again.
- **Node 22 is the floor.** `engines.node` is `>=22.0.0`. It said 18, while commander 14 and open 11 need Node 20 or later, so every package built on faf-cli inherited a false floor. CI and the release matrix test Node 22 and 24.

### Fixed
- **Every writer is non-destructive, with no exceptions.** No faf writer replaces, reorders, truncates or deletes text it cannot prove it wrote. It touches only its own block or its own section; when it cannot prove that, it refuses in one line and the file stays byte for byte. Every write in `src/` goes through one module, `src/core/safe-write.ts`, and a test in the suite (`tests/write-guard.test.ts`) fails if any other file calls a filesystem API that writes, appends, renames, deletes, makes a folder, links or changes a mode. Two commands write through a subprocess, which that test cannot see, and their effects follow the same rule: `faf diff --install-driver` (`git config`) and `faf git` (a clone into faf's own temp folder). `faf diff --uninstall-driver` edits the repo's git config file through `src/core/safe-write.ts`, like every other writer.
- **A marker example is never taken for faf's block.** faf read fences with a plain on/off toggle, so a ```` fence around a ``` line, a `~~~` fence, a ``` line with a language after it, an indented ```, a fence never closed, a fence in a list item, `<pre>`, `<?php … ?>`, `<![CDATA[ … ]]>` or a multi-line HTML comment made it take a marker example inside for the block and replace the text around it. faf now reads every block file two ways — as CommonMark reads it (list items and block quotes included) and plainly, every line at column 0 — and takes a block as its own only when both readings find the same start and end lines, outside every code block, raw HTML block and comment. A marker line must be exactly the marker. With two start lines before an end line, the block is the last start before the first end, so a start line you typed above faf's block no longer pulls your text into it. When a `.faf` value would leave a fence open inside faf's own block, faf closes it at the end of the block (or quotes the block's lines with `> `), so the block is always found again. Both readings take time linear in the file's length, at any nesting depth. This covers CLAUDE.md, AGENTS.md, GEMINI.md, `.cursorrules`, the Copilot file, MEMORY.md, `llms.txt`, `injectFafBlock` and claude-faf-mcp's earlier section in Claude Code's MEMORY.md.
- **faf never reads or writes through a link that leads out of the project.** With `project.faf → ~/.aws/credentials` in a cloned repo, the secret was read as project context; `CLAUDE.md → ~/.zshrc` got faf's block written into the shell config. faf now refuses a link that leaves the project, and a dangling link, before it reads or writes anything. Inside the project, faf follows a link only to the same kind of file: one with the same name (`CLAUDE.md → docs/CLAUDE.md`), or another AI context file (`CLAUDE.md → AGENTS.md`); `CLAUDE.md → README.md`, `CLAUDE.md → package.json` and `project.html → .git/config` are refused. When faf reads project context through a link, the link must end at a `.faf` or `.fafm` file — or, for an AI context file, at another AI context file — so `project.faf → .env` is refused too. Anything inside `.git` is refused, with two allowances: faf's section of a hook file directly in the repo's hooks folder, and the driver section faf wrote in the repo's own git config file (`faf diff --uninstall-driver`). faf makes `.github`, `.github/workflows`, `.grok` and `.well-known` one folder at a time inside the project and never creates a folder through a link; Claude Code's memory folder and a `faf export --output` folder, which are the write's own root, are made as named. The refusal is one line that names the link and its target.
- **A failed write never leaves a half-written file.** Every writer writes a temp file next to the target, flushes it to disk and renames it over the original in one step, keeping the original's permissions. On a full disk, a quota or a killed process the original stays exactly as it was, and the error says "<file>: not written; original kept". A read-only file is not replaced.
- **faf never writes over an edit made after it read the file.** An edit to soul.fafm while a soul was loaded, to project.faf while `faf auto` was detecting, to `.grok/config.toml` while `faf export --grok` ran, or another `faf` process's growth in `.faf-dna` was lost to the later write. Every writer now reads the file again just before the rename; if its bytes or its mode changed, nothing is written: "<file> changed on disk while faf was writing — not written; original kept". A file that appeared while faf was creating one is not written over either: `FafDNAManager.birth` refuses a `.faf-dna` that appeared after the manager found none. Over one that was there when it looked, `birth` starts a fresh lineage, as `faf init --force` does.
- **A file that is not UTF-8 is refused, never re-encoded.** A CLAUDE.md saved as "Unicode" (UTF-16) by Notepad or PowerShell 5, or a project.faf, soul.fafm, MEMORY.md, `.faf-dna` or `.grok/config.toml` with a cp1252 `é`, was read with those bytes turned into U+FFFD, and the next write put U+FFFD where your bytes had been. faf now reads these files strictly as UTF-8 and otherwise refuses: "<file> is not UTF-8 — faf left it unchanged". A UTF-8 BOM works as before.
- **A project.faf that is not valid YAML is one line, never a stack trace.** `faf auto`, `faf export`, `faf sync`, `faf go`, `faf check` and the other commands that read project.faf through faf's .faf reader printed yaml's stack trace for a repeated key; they now print "<file> is not valid YAML (Map keys must be unique, line 8) — faf left it unchanged" and exit 1. `readFaf`, `writeFaf` and `updateFafFile` throw a `SafePathError` with reason `not-yaml` (yaml's error is its `cause`).
- **A project.faf that is a scalar or a list is one line in every command that reads it.** A file that parses to `just a string` or `- a` was read as a project with nothing in it: `faf score` and `faf status` said 0%, `faf compile` compiled it, `faf refresh` started a `.faf-dna` for it, `faf sync`, `faf export` and `faf show` wrote their files from it, `faf auto` wrote a new project.faf over it, and `faf edit` stopped with a stack trace on a string and rewrote a list as a mapping (`"0": a`). Every command that reads project.faf through faf's .faf reader now refuses it before it does anything else, in one line — "<file>: a .faf must be a YAML mapping (key: value pairs), but this one is a string ("just a string"). faf left it unchanged — fix it by hand." — exits 1 and writes nothing (no `.fafb`, no `.faf-dna`). `faf check` exits 1 with that line too, as it does for a file that is not valid YAML; its exit 3 stays its verdict on a .faf it can read (a required field missing, or text the kernel does not validate) — an empty project.faf reads as empty and gets exit 3 for its missing `faf_version` and `project.name`. `faf diff` and `faf log` still score such a version as 0, as they do any version they cannot read. In the library, `readFaf`, `writeFaf` and `updateFafFile` throw the same `SafePathError` (`not-yaml`); `updateFafFile` throws it before it calls `mutate`.
- **A scoring-kernel rejection is one line in every command.** `faf score`, `faf status`, `faf compile`, `faf refresh` and `faf sync --direction pull` handed project.faf's text to the kernel before reading it, so a repeated key printed the kernel's own error as a Node crash report (`triggerUncaughtException`, then the kernel's message). They now read it first, print the same one line as the other commands and exit 1. Text that yaml reads but the kernel cannot (a 30-digit integer, nesting past the kernel's depth limit) printed a stack trace in those commands and in `faf sync`, `faf export --html`, `faf show`, `faf taf`, `faf loop` and `faf bench`, and so did a `.fafb` the kernel cannot read (cut short) in `faf decompile`. In every command it is now one line — "<file>: faf's scoring kernel could not read it (<reason>) — faf left it unchanged" — and exit 1 (`faf check` keeps its own verdict, exit 3). `faf auto`, `faf go`, `faf loop` and `faf sync --direction pull` ask the kernel before they write, so such a project.faf stays byte for byte: 7.12's `faf auto` rewrote it (a 30-digit integer rounded), and `faf go` asked its questions first. `faf diff` and `faf log` score such a version as 0, as before.
- **A project.faf that starts with a BOM scores, compiles and checks.** `faf score` stopped with "more than one document" on such a file with more than one top-level key. The score now leaves a leading BOM out — in `faf score`, in `faf auto` (which now keeps the BOM when it edits the file) and in the library's `kernel.score` and `kernel.scoreEnterprise` — and the file keeps its BOM. `faf compile`, and `faf refresh` when it re-compiles a `.fafb`, stopped the same way on such a file, and `faf check` called it "invalid yaml"; they leave the BOM out now too, as `kernel.compile` and `kernel.validate` do.
- **Writing project.faf keeps everything faf did not change.** Every write rebuilt the whole file: comments were lost, `version: 1.10` became `1.1`, a 20-digit number was rounded, `0x1F90` became `8080`, `&defaults` became `&a1`, and your own `_meta` was dropped. `faf auto`, `faf edit`, `faf go`, `faf migrate`, `faf recover`, `faf conductor` and `faf sync --direction pull` now change an existing project.faf in place and rewrite only the values that change: comments, blank lines, key order, quoting, number text, anchors, unknown keys, CRLF line ends, a BOM and the `%YAML` / `---` / `...` framing all stay. A run that changes nothing writes nothing, and `faf auto` then says `unchanged`. A project.faf that is not valid YAML, or that parses to a scalar or a list, is refused instead of being overwritten; an old `project: <name>` is lifted to `project: { name }`. `faf init --force` and `faf git --force` still write a fresh file.
- **An alias is never replaced, and an anchored value that aliases read is never filled.** With `stack: *base`, `faf auto` replaced the alias with a copy of the map; with `goal: &g Old goal` and `summary: *g`, `faf edit project.goal` wrote `summary: Old goal` over the alias. faf now leaves every alias as written: a value `data` only repeats is never written, a change under an alias is not written, and faf prints one line ("stack is an alias (*base) — faf left it as written"; `faf edit` of an alias key refuses with it and exit 1). A change to the value an alias points to still lands, and the alias follows it. `faf auto` never fills a value with an anchor that an alias reads (`frontend: &x None` while `ui_library: *x`), since that would change every alias: it leaves the value as written and says "stack.frontend holds an anchor (&x) that an alias reads — faf left it as written". Editing a value with an anchor or a tag changes only its value text; the anchor, the tag and the line's spacing stay.
- **A comment on a value faf fills in or lifts to a section is kept.** `project: demo # note` lifted to `project: { name: demo, … }`, and `stack: # decide after ADR-7` filled, both lost the comment. It now stays as a comment line above the new entries, or on the same line when the new value is empty; a value replaced by a different one keeps its line comment. The same holds for soul.fafm (`memory: # note`, `facts: # note`).
- **`faf auto` adds `# found:` only next to a type it fills in** — never next to a `type:` you wrote.
- **`faf auto` fills a slot under the name your file uses, from the repo's fact.** A repo fact (a `postgres` image in docker-compose) fills `stack.db: None` itself. faf also keeps `stack.database` — the name the scoring kernel reads — in step with it (the fact, or `slotignored` in a slot your app-type leaves out), so the score counts what your file says; with no fact, neither name is written. A slot your app-type uses got `slotignored` when detection read the repo as another type; it now gets the repo's fact (`runtime: Go` from go.mod), or keeps what it had.
- **`faf go` refuses an answer it cannot place**, naming the blocking field, when a step on the path is not a mapping, instead of replacing it.
- **`faf go --resume` works.** Quitting the interview saved the session and then deleted it at the end of the same run. The session is now kept on quit and removed when the interview is finished — only when it is the session faf wrote.
- **Saving a soul.fafm keeps what the file had.** A save rebuilt the file: a hand-kept `index` was replaced, comments were lost, `version: 2.0` became `1.1`, and a `profile: voice` the file never had was added. `FafmSoul.save` now edits the loaded file in place and writes only what changed (new facts, changed fields, `last_etched`); a save that changes nothing writes nothing. A plain `save(path)` rebuilds the index only when it was exactly the one faf derives from the facts and was not changed in memory since; a hand-kept index is never touched (`{ reindex: true }` always rebuilds, `{ reindex: false }` never does). `memory.facts`, `index` or `sessions` written as a mapping, and list items that are not facts, now load and are kept as written; a change that would rewrite one of those parts is refused.
- **Etching an existing id updates that fact instead of replacing it.** Its text and timestamp change; links, source, tags, type, priority and extra fields are kept unless you pass new ones, and a priority is never lowered unless you pass one. `add()` still replaces the whole fact, as the .fafm INTEROP contract says.
- **`FafmSoul.recall` accepts the options object** (`recall({ query, tags, type, minPriority, limit })`) that MCP servers call it with, instead of throwing `toLowerCase is not a function`, and a bare-string fact passed in from code becomes a proper fact with text.
- **A `.faf-dna` written by another tool no longer breaks `faf auto` or `faf dna`.** claude-faf-mcp's faf_dna wrote `.faf-dna` in its own shape; with that file committed, `faf auto` exited 1 after writing project.faf, and `faf dna` exited 1. faf now reads such a file as far as it can and never rewrites it; a `.faf-dna` that is not JSON, or has no birth certificate, reads as no DNA. Growth is added only to a `.faf-dna` whose text is exactly what faf writes for it (2-space JSON and a final newline), with no key of yours in an entry faf replaces and faf's own label and emoji on the peak and current milestones; any other is left byte for byte, and `faf auto`, `faf refresh` and `faf dna` say why in one line. Before, a 20-digit number was rounded, keys were reordered, a repeated key lost its first value and a note on the peak milestone was dropped.
- **`faf server-card` edits server.json in place and keeps its layout.** It rebuilt the file from parsed JSON in faf's own key order, so a 20-digit `buildId` was rounded, a one-line `packages` array was re-laid out, a hand `title` was deleted when project.faf had none, and other keys under the registry's publisher-provided `_meta` were dropped. Now only faf's identity changes, as text: `name`, `title` (only when project.faf has one), `version` (with `--set-version`) and the keys of faf's context-block. A key faf adds goes on a line of its own (a missing `title` after `name`, a missing `_meta` before the last key); everything else stays where the file has it, and no field is deleted. `--check` prints the edited file. A server.json faf cannot edit that way — not valid JSON, a repeated key, a `_meta` that is not an object, or an object or list where faf would put a plain value (a `title` of `{"en": …}`) — is refused in one line. `faf cards --target registry` makes the same edit.
- **`faf server-card` and `faf cards` no longer fail with "context block drifted".** With no timestamp in project.faf or in the file, each card read the clock on its own, and a run that crossed a millisecond threw. One timestamp is used for the whole run.
- **`faf cards` writes nothing when nothing changed.** Every run wrote its own time into faf's context block in the Server Card and server.json, so both changed on every run. `faf cards` now keeps the time stamp the cards already carry — server.json's first (the one `faf server-card` keeps there, or sets at release), then the Server Card's, then the A2A card's — as `faf server-card` does, and lists a card it did not need to write as `(unchanged)`.
- **`faf cards --target catalog` never touches a row that is not faf's.** It matched rows by type and URL, so your row for another agent's A2A card had its URL and date replaced. faf now updates only the row whose identifier is exactly its own (`urn:air:<host>:a2a:<name>`, `urn:air:<host>:agent:<name>`) — only its url, type and updatedAt — appends its row otherwise, and edits the catalog as text, so every other row and byte stays. `upsertCatalog` matches by exact identifier too.
- **`faf compile` never writes the `.fafb` over its source.** For `project.FAF` or `notes.fafm` the default output was the source's own path, and `--force` replaced the source with binary. The output is now the name with its `.faf` ending (any case) turned into `.fafb`, or `.fafb` added to any other name, and an output that is the source itself (the same path, a case variant or a link to it) is refused, `--force` included. `faf refresh` leaves a `.fafb` that is not faf's as it is, says so in one line, and carries on.
- **`faf decompile --output <path>` writes the file.** The help offered `--output`, but faf ignored it and printed the JSON. It now writes the JSON to that path, carrying faf's render hash (`_meta["one.faf/render"]`) like every whole file faf writes: a file already there is replaced only when it is exactly what faf last wrote there, anything else is refused in one line and left as it is unless you pass `--force`, and the output never goes over the `.fafb` being read.
- **An output folder that is not there is one line.** `faf compile --output`, `faf taf --output`, `faf init --output` and `faf server-card --out` printed a stack trace when the folder in the path did not exist. They now print "<folder> does not exist — nothing written" (or "<folder> is not a folder — nothing written" when a file has that name), write nothing and exit 1, and so do `faf decompile --output` and `faf git --output` (`faf git` before it clones). faf does not create that folder; `faf export --output` still does, as documented: that folder is the export's own.
- **`faf diff --install-driver` and `--uninstall-driver` touch only faf's own driver.** Install set `diff.faf.command` over a value of yours; uninstall (`git config --unset`) removed any value, and your comment on faf's line, or the key written on the `[diff "faf"]` header line, went with it. Install now sets the command only when it is unset or already `faf-cli diff-driver`, and only when no other `diff.faf.*` setting of yours (a `textconv`) is there. Uninstall removes only the section install writes — a `[diff "faf"]` line and a tab-indented `command = faf-cli diff-driver` line, nothing else in it — as a text edit of the repo's own config file, checked with git's own reading of the result. Anything else is left alone with one line and exit 1, and `.gitattributes` is left as it is. A config file with a carriage return that does not end a line is never edited: faf refuses it in one line (exit 1), or, when git reads faf's section as part of a comment (`# note\r[diff "faf"]`), says the driver is not set and changes nothing.
- **`faf hooks` touches only its own section of your pre-commit hook.** A hook that quoted the marker text, or had a start marker with no end, had the text in between replaced. Markers now count only as whole lines, one start before one end; any other shape is refused in one line. Install adds faf's section to a hook it did not write, or updates the section it did; uninstall removes only that section; the hook keeps its mode (execute is added only where read is set). In a hooks folder linked outside the repo, faf only creates a new hook or updates its own section, and a pre-commit that is a link out of the hooks folder is refused. Uninstall also removes what install added around faf's section — the blank line between your lines and the section, or, in a hook that is blank outside the section (empty, or only blank lines), the `#!/bin/sh` line install put on top — so install and then uninstall leave your hook byte for byte (a last line of yours with no line end keeps the one install added). A pre-commit faf created where there was none is left as an empty file.
- **`faf recover` reads CLAUDE.md, AGENTS.md and GEMINI.md the way faf reads project context.** An `AGENTS.md` linked to a file outside the project had its first heading written into project.faf as the project name, and a cp1252 `é` became U+FFFD. A refused source is now skipped with its one-line reason; when every source is refused, nothing is written and the exit code is 1. It still reads through a link from one AI context file to another (`CLAUDE.md → AGENTS.md`), the rule the writers use; a link to any other file (`CLAUDE.md → README.md`) is refused.
- **`faf clear` removes only temp folders faf made.** It removed every `faf-git-*` folder in the temp folder, so a folder of yours named `faf-git-my-notes` went with everything in it. A folder is removed now only when its name is exactly faf's (`faf-git-` and six letters or digits) and it holds the `.faf-temp` marker faf writes into each temp folder it makes. `faf git` clones into a `repo` folder beside that marker. `faf-git-*` folders left by 7.12 or earlier have no marker, so `faf clear` leaves them; remove them by hand.
- **A refusal or a failed write is one line in every command, never a stack trace**, including `faf go`, `faf ai` and `faf hooks --install` over a read-only hook. Where the reason does not already say what faf left alone, the line ends with it: "Nothing was written to it." when faf had read the file first (`faf auto` with `project.faf → config/team.faf`), "Nothing was read from or written to it." when it refused before either.
- **Detection never looks above the project folder.** The format finder walked up to 10 parent folders, so `faf init` / `faf auto` in a new folder, a subfolder or a monorepo package wrote the stack of the tree above it (a parent's `tsconfig.json`, `vercel.json` or `Cargo.toml`) into project.faf and CLAUDE.md. It reads the project folder only, a folder named like a format (`go.mod/`) no longer counts as that format, and a README no longer adds the fixed guesses "developers", "understanding project" and "project documentation".
- **An About Repo with no `source_score` shows "unknown", not "-1%".** `scoreFafYaml` marks the result `unknown: true`; `faf score` and `faf score --status` print `unknown` with no tier, and `faf refresh` records no -1 Birth DNA.
- **The home and filesystem-root guard compares identity, not spelling** (device + inode), so a case-variant path, a symlink to home or a macOS firmlink is refused like home itself.
- **`faf sync` and its docs say which way it writes.** The help said "mtime auto-direction", and the docs said "bi-sync" and "edit either; the other follows"; since 6.6.0 `faf sync` has written CLAUDE.md from `.faf`, never the other way. The help, the README and the docs, the faf-expert skill included, now say so: `faf sync` writes `.faf` → CLAUDE.md, tri-sync writes `.faf` → MEMORY.md, and `faf sync --direction pull` is the only way sync writes back into `.faf`, only at ✪ Trophy. They also say that `faf git` makes a shallow clone of the repo, where they said "no clone".
- **tri-sync writes the MEMORY.md Claude Code actually loads.** `faf sync` with tri-sync on (`FAF_PRO=1`) wrote `<project>/MEMORY.md`, a file Claude Code never reads, and printed "MEMORY.md → .faf" on runs that did nothing. It now writes faf's block into `~/.claude/projects/<id>/memory/MEMORY.md`, keeps every line Claude wrote there, and says what it did (created, updated, block added on top, unchanged). One direction only: `.faf` → MEMORY.md.
- **`faf export --help` names the card file faf writes:** `./server-card`, not `.well-known/mcp/server-card`.
- **`faf auto --help` says what `faf auto` does:** "Fill every tech slot from the repo, then score", where it said "Zero to 100% in one command" — 100% needs your 6Ws (`faf go`). The README says that `faf` with no arguments shows your project's score and `faf auto` detects and fills, where it said bare `faf` was `faf auto`, and the agent guides it links run `bunx faf auto` where they ran `bunx faf`.

### Added
- **Safe file access**, the rules every faf writer uses, so MCP servers built on faf-cli route their own reads and writes through them: `resolveInside(dir, name, { read })`, `safeWriteFile(path, content, { root, expect, mode, allowGitHooks, allowGitConfig })` (`expect`: the bytes the caller read, or `null` for no file; the write is refused if the file no longer holds them), `readUtf8(path)` (strict UTF-8), `safeReplaceOwned(path, content, { owns, mark, force, root, expect })` (a whole file, only over one faf owns), `makeDirInside(root, dir)` and `safeUnlink(path, { expect })`, with the `ResolveInsideOptions`, `SafeWriteOptions`, `ReplaceOwnedOptions` and `SafePathReason` types. `SafePathError` carries a `reason` — `outside`, `dangling`, `not-a-file`, `not-faf`, `other-file`, `git`, `not-utf8`, `changed`, `not-owned`, `not-yaml` or `unplaceable` — and an `onWrite` flag (true when faf refused at the write itself, after it may have read the file).
- **Document-preserving .faf updates:** `updateFafFile(path, mutate)` parses a .faf with yaml's `parseDocument`, lets `mutate` change the Document and writes back only what changed; it returns an `UpdateFafResult` (`{ written, text, keptAliases }`). `writeFaf` takes `WriteFafOptions`: `{ replace }` for an explicit overwrite and `{ onAliasKept }`, called with a `KeptAlias` (`{ path, alias, kind }`, `kind: 'anchor'` for an anchored value a fill left alone) for each path a write left as written.
- `isExplicitNone(value)`, `EXPLICIT_NONE` and `SLOTIGNORED`: a typed none counts as an empty slot, and `fillEmpties` and `updateExistingFaf` handle it — and every other word typed into a slot — as `faf auto` does.
- `injectFafBlock(path, block, start, end, { root })` (the `InjectOptions` type) keeps a file in a subfolder (`.github/…`) inside the project folder. `legacyStampNote(label, text)` and `legacyStampNoteAt(path, label)` return the one line `faf sync` and `faf export` print when faf's block goes on top of older faf text — the old stamp, an older block in a code fence, a raw HTML block or a comment, or one past more than 100 nested lists or quotes.
- **Claude Code's MEMORY.md:** `writeClaudeMemory(dir, data)` writes faf's block into the MEMORY.md Claude Code loads for a project and returns what it did (`created`, `updated`, `migrated`, `added` or `unchanged`), whether every byte outside the block is still there, and warnings (past the 200 lines Claude Code loads; the block went on top of older faf text — the `legacyStampNote` line). `resolveClaudeMemoryPath(dir)`, `resolveClaudeMemoryDir(dir)`, `claudeProjectRoot(dir)` and `claudeProjectId(root)` follow Claude Code's own rule for that path (the canonical git root; every character outside `[a-zA-Z0-9]` turned into `-`; a cut at 200 characters plus a hash; `CLAUDE_CONFIG_DIR`). `renderClaudeMemory(data)` returns the block body, and `claudeMemoryStatus(dir)` reads the file without writing. Types: `ClaudeMemoryOptions`, `ClaudeMemoryResult`, `ClaudeMemoryAction`, `ClaudeMemoryStatus`.
- **Cards:** `patchServerJson(text, { name, title, version, meta })` (the `ServerJsonIdentity` type) — the text-preserving identity edit `faf server-card` makes — and `registryTitle(data)`, the registry display title from `project.title` (trimmed, at most 100 characters). The `ProjectHtmlWriteOptions` and `CardWriteOptions` types: the `{ force }` options `writeProjectHtml` and `writeServerCard` take.
- **`faf git` without git:** `normalizeGitUrl`, `repoNameFromUrl` and `authorFafFromRepo(dir, { repoUrl })` (the `AuthorFafFromRepoOptions` type) return the FafData `faf git` would write for a repo you fetched your own way. Nothing exported runs git.
- **FAF DNA:** `FafDNAManager` (birth, recordGrowth, getJourney — the API `faf init`, `faf auto` and `faf dna` use), with `isFafShape()` (a `.faf-dna` faf may add to) and `readOnlyReason()` (why one is only read), and the `FafDNA`, `BirthCertificate`, `VersionEntry` and `Milestone` types.
- **Souls:** `FafmSoul.derivedIndex()` (the index faf would derive from the facts) and `FafmSoul.indexIsDerived()`; `FafmSoul.save` / `toFile` take `{ replace, reindex }`; the `FafmRecallOptions` type.
- `scoreText(result)`: `85%`, or `unknown (—)` for a result marked `unknown` — render such a result as unknown, never as a number. `isNonProjectRoot(dir)`: the home and filesystem-root check `faf init`, `faf auto` and `faf go` refuse on.
- **CLI:** `--force` on `faf show`, `faf export`, `faf cards`, `faf server-card` (with `--out`), `faf compile`, `faf taf` (with `--output`) and `faf decompile` (with `--output`) replaces a file faf cannot prove it wrote; `faf memory convert --force` converts over an existing soul.fafm.

### Removed
- **`faf ai enhance` is retired.** project.faf isn't enhanced: a tech slot holds a repo fact, a 6W holds your own words, and an AI suggestion is neither. `faf ai enhance` prints one line ("faf ai enhance was retired in 7.13 — project.faf isn't enhanced: faf auto fills tech slots from repo facts, and you write the 6Ws (faf go)."), writes nothing and exits 1. Use `faf auto` for tech slots and `faf go` for the 6Ws. `faf ai analyze`, which only reads project.faf and prints suggestions, stays, with the optional `@anthropic-ai/sdk` it uses.

### Maintenance
- **The lint gate lints every file.** `eslint src/**/*.ts` ran unquoted, so the shell expanded the glob two folders deep and `src/cli.ts` and `src/index.ts` were never linted; the glob is quoted.
- **`npm run check:engines` keeps the Node floor true.** It fails CI and `prepublishOnly` when any Node version in any workflow is below `engines.node`, or when the lowest one is not the floor. It reads every `node:` / `node-version:` value (a list, a single value or an `env` reference) and the first line of the file a `node-version-file:` names, and fails on a version it cannot read (`lts/*`, `latest`, a version file that is not there).
- **Two test suites no longer write into the repo.** tests/fafm/fafm.test.ts and tests/wjttc/memory-cli.test.ts wrote into `tests/.tmp-fafm*`; they now use temp folders of their own (and a HOME of their own), removed when they end.

### Notes
- Every file faf rewrites gets a new inode: faf writes a temp file and renames it over the original. Another hard link to the old file keeps the old bytes, and extended attributes (a Finder comment, `user.*` attributes) are not carried over. faf keeps the file's mode, and its owner where the system allows.
- Moving claude-faf-mcp's old tri-sync section in MEMORY.md into faf's block replaces that whole tool-managed section, from its heading line to its closing line, including any line typed inside it. The notes around it stay byte for byte.
- A project.faf written in JSON syntax stays valid after a faf edit, but faf's additions use YAML flow style (unquoted keys and values) rather than JSON.
- When `faf auto` (or another faf edit) changes a flow-style list or map that holds a comment — `tags: [a, # note` on one line, `b]` on the next — it may re-lay out that list or map, with the brackets and each item on lines of their own. Every item and every comment is kept.
- In a pre-commit hook, a line that is exactly `# >>> faf >>>` and a later line that is exactly `# <<< faf <<<` are faf's section wherever they sit, a heredoc of yours included: `faf hooks --install` replaces the lines between them and `--uninstall` removes them. Keep those two exact lines out of your own hook text.
