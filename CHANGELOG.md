...
- **One-time step for an existing project.html, Server Card, A2A card, `faf server-card --out` file or `faf taf --output` snapshot: faf replaces a whole file only when it is exactly what faf last wrote.** Each of these files now carries a hash of faf's own render: project.html a `<meta name="faf-render" content="sha256:…">` line (the hash of the page without that line), the JSON files `_meta["one.faf/render"]` (the hash of the file without that key). The MCP Server Card and registry server.json schemas leave `_meta` open for namespaced keys (the registry keeps only its publisher-provided key on publish). The A2A Agent Card has no `_meta` field, and the A2A spec says a reader should ignore a field it does not know, as the official Python and Go SDKs do. On each write faf recomputes the hash:
  ...
  - it does not match: the file was edited since, and faf leaves it: "<file> was edited since faf wrote it — faf left it unchanged. Use --force to replace it.";
  ...
  Check a refused file for hand edits you want to keep, then run the same command once with `--force`, which writes faf's render over the whole file, text and all. A `faf export` or `faf cards` run that refuses one file still writes the others, prints one line per refused file and exits 1. A file git checked out with CRLF line ends is no longer byte for byte faf's, so it is refused the same way. In the library, `writeProjectHtml`, `writeServerCard` and the `faf cards` writer throw a `SafePathError` with reason `not-owned` in these cases and change nothing; pass `{ force: true }` to replace the file on purpose. A `.fafb` is a compiled binary: faf replaces it only when it starts with the FAFB header.
...
- **`faf diff --install-driver` and `--uninstall-driver` touch only faf's own driver.** Install set `diff.faf.command` over a value of yours and uninstall removed any value. faf now reads `git config --get-all diff.faf.command` first: install sets it only when it is unset or already `faf-cli diff-driver`, and uninstall unsets it only when its one value is exactly that. Anything else is left alone with one line and exit 1, and `.gitattributes` is left as it is too.
...
- **`faf recover` reads CLAUDE.md, AGENTS.md and GEMINI.md the way faf reads project context.** An `AGENTS.md` linked to a file outside the project had its first heading written into project.faf as the project name, and a cp1252 `é` became U+FFFD. A refused source is now skipped with its one-line reason; when every source is refused, nothing is written and the exit code is 1.
...
### Added
- **Safe file access**, the rules every faf writer uses, so MCP servers built on faf-cli route their own reads and writes through them: `resolveInside(dir, name, { read })`, `safeWriteFile(path, content, { root, expect, mode, allowGitHooks })` (`expect`: the bytes the caller read, or `null` for no file; the write is refused if the file no longer holds them), `readUtf8(path)` (strict UTF-8), `safeReplaceOwned(path, content, { owns, mark, force, root, expect })` (a whole file, only over one faf owns), `makeDirInside(root, dir)` and `safeUnlink(path, { expect })`, with the `ResolveInsideOptions`, `SafeWriteOptions`, `ReplaceOwnedOptions` and `SafePathReason` types. `SafePathError` carries a `reason` — `outside`, `dangling`, `not-a-file`, `not-faf`, `other-file`, `git`, `not-utf8`, `changed` or `not-owned` — and an `onWrite` flag (true when faf refused at the write itself, after it may have read the file).
