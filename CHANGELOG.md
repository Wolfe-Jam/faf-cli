...
- **Every writer is non-destructive, with no exceptions.** No faf writer replaces, reorders, truncates or deletes text it did not write:
  - A file with no block markers is never taken over, whatever its first or last line says. A CLAUDE.md, AGENTS.md, GEMINI.md, `.cursorrules` or Copilot file that started with faf's two-line stamp was erased whole, so a hand-written file in that shape lost every section. faf now puts its block on top and keeps every byte; the only text it replaces is what sits between its own markers.
...
- **faf no longer reads or writes through a link that leads out of the project.** When `project.faf`, `.faf`, `soul.fafm`, CLAUDE.md, MEMORY.md, or any of the files in the next entry is a symlink to somewhere outside the project folder, faf refuses and reads or writes nothing. Before, a cloned repo with `project.faf → ~/.aws/credentials` had the secret read as project context, and `CLAUDE.md → ~/.zshrc` got faf's block written into the shell config. A dangling link is refused instead of creating the file at its far end. A link that stays inside the project (`CLAUDE.md → AGENTS.md`) still works and stays a link. When faf reads project context through a link, the link must also end at a `.faf` or `.fafm` file, so `project.faf → .env` is refused too.
- **A failed write no longer leaves a half-written file.** The writers for `.faf`, CLAUDE.md, AGENTS.md, GEMINI.md, `.cursorrules`, the Copilot file, MEMORY.md, `llms.txt`, `soul.fafm`, `.faf-dna`, `project.html`, the Server Card, `.grok/config.toml`, the `faf cards` output, `project.fafb`, `server.json`, `taf.yml`, `.gitattributes`, the `faf go` session, the `faf bench` state and faf's git hook write a temp file next to the target, flush it to disk and rename it over the original in one step, keeping the original's permissions. On a full disk, a quota or a killed process the original is left exactly as it was, and the error says "not written; original kept". Before, the file was cut short and the user's text after the cut was gone. A read-only file is not replaced.
- **Writing project.faf keeps everything faf did not change.** Every write used to rebuild the whole file: comments were lost, `version: 1.10` became `1.1`, a 20-digit number was rounded, `0x1F90` became `8080`, `&defaults` became `&a1`, and a user's own `_meta` was dropped. A write that changed nothing still rewrote the file. `faf auto`, `faf edit`, `faf go`, `faf migrate`, `faf recover`, `faf conductor` and `faf sync --direction pull` now change an existing project.faf in place. Only the values that change are rewritten, so comments, blank lines, key order, quoting, number text, anchors, unknown keys, CRLF line ends and a BOM all stay. A run that changes nothing writes nothing, and `faf auto` then says `unchanged`. A key the file has and the new data leaves out is kept. A project.faf that is not valid YAML is refused instead of being overwritten. `faf init --force` and `faf git --force` still write a fresh file, because that is what `--force` asks for.
- **`faf auto` no longer adds `# found:` to a type you wrote.** The detection note is added only next to a `type:` that faf itself fills in.
- **A typed None / N/A / not applicable is an empty slot: it scores 0 until filled — the app-type decides which slots count.** `faf auto` fills it when the repo has the fact; with no fact your words stay as typed, and faf never rewrites them to `slotignored` (7.12 did, which lifted the score). The 6Ws stay yours: `faf go` asks.
- **Saving a soul.fafm keeps what the file had.** A save used to rebuild the file. A hand-kept `index` was replaced by lines faf wrote itself, comments were lost, `version: 2.0` became `1.1`, and a `profile: voice` the file never had was added. `FafmSoul.save` now edits the loaded file in place and writes only what changed: new facts, changed fields and `last_etched`. A save that changes nothing writes nothing.
- **Saving a soul.fafm rebuilds the index only when faf wrote it.** A plain `save(path)` keeps an index in step with the facts only when it was exactly the one faf derives from them when the soul was loaded or made, and was not changed in memory since; a new soul counts as faf's unless you gave it an index of your own. A hand-kept index is never touched. `save(path, { reindex: true })` always rebuilds it and `{ reindex: false }` never does. A new soul with two facts saves an index of two, as it did in 7.12. `faf memory etch` works the same way.
- **A soul.fafm with parts faf does not model still loads, and those parts are kept as written.** This covers `memory.facts`, `index` or `sessions` written as a mapping, and list items in `memory.facts` that are not facts. Before, a mapping-shaped `index` or `sessions` was dropped on the next save, and a soul with mapping-shaped facts or a non-fact list item did not load at all. A change that would rewrite one of these parts, such as adding a fact to a mapping-shaped `memory.facts`, is refused, and nothing is written.
- **Etching an existing id updates that fact instead of replacing it.** The text and timestamp change; links, source, tags, type, priority and extra fields are kept unless you pass new ones. A priority is never lowered unless you pass one. Before, etching `id: pay` dropped the fact's links, source, tags, extra fields and type, and demoted a critical fact to standard. `add()` still replaces the whole fact, as the .fafm INTEROP contract says.
- **`FafmSoul.recall` accepts the options-object form** (`recall({ query, tags, type, minPriority, limit })`) that MCP servers call it with, instead of throwing `toLowerCase is not a function`. A bare-string fact passed in from code is now a proper fact with text, instead of showing as `undefined`.
...
- **Detection never looks above the project folder.** The format finder (Turbo-Cat) walked up to 10 parent folders and stopped only at the project's own `.git`, so `faf init` / `faf auto` in a new folder, a subfolder or a monorepo package wrote the stack of the tree above it (a parent's `tsconfig.json`, `vercel.json` or `Cargo.toml`) into project.faf and CLAUDE.md. It now reads the project folder only. A folder named like a format (`go.mod/`) no longer counts as that format, and a README no longer adds the fixed guesses "developers", "understanding project" and "project documentation".
- **A `.faf-dna` written by another tool no longer breaks `faf auto` or `faf dna`.** claude-faf-mcp's faf_dna wrote `.faf-dna` in its own shape (no `versions`, no `growth`). With that file committed, `faf auto` exited 1 after it had already written project.faf, and `faf dna` exited 1, for everyone on the repo. faf now reads such a file as far as it can and never rewrites it: `faf dna` shows the journey and says the file is left as it is, and growth is recorded only into a file in faf's own shape. A `.faf-dna` that is not JSON, or has no birth certificate, reads as no DNA. A `.faf-dna` link that leads out of the project is not read.
- **An About Repo with no `source_score` shows "unknown", not "-1%".** `scoreFafYaml` now marks the result `unknown: true`. `faf score` and `faf score --status` print `unknown` with no tier, and `faf refresh` says the score is unknown and records no -1 Birth DNA.
- **Pro tri-sync writes the MEMORY.md Claude Code actually loads.** `faf sync` with Pro wrote `<project>/MEMORY.md`, a file Claude Code never reads, and printed "MEMORY.md → .faf" on runs that did nothing. It now writes faf's block into `~/.claude/projects/<id>/memory/MEMORY.md`, the file Claude Code loads for the project (see Added). Every line Claude wrote there is kept byte for byte, and the output says what happened (created, updated, block added on top, unchanged). Tri-sync goes one way only: .faf → MEMORY.md.
- **A link inside the project leads faf only to the same kind of file.** faf used to write through any link that stayed inside the project. `CLAUDE.md → README.md` got faf's block on top of the README, `CLAUDE.md → package.json` left JSON that no longer parsed, `.cursorrules → .git/config` broke git, and `project.html → .git/config` replaced git's config. Now faf writes through a link only when the file at the end has the same name (`CLAUDE.md → docs/CLAUDE.md`), or when both are AI context files (`CLAUDE.md → AGENTS.md` still works). project.html, the Server Card, the `faf cards` JSON, `.grok/config.toml`, `.faf`, `.fafm` and `.faf-dna` are written through a link only to a file of the same name. Anything inside `.git` is refused, always. The refusal is one line that names the link and its target, and nothing is written.
- **A file that is not UTF-8 is refused, never re-encoded.** A CLAUDE.md that Notepad or PowerShell 5 saved as "Unicode" (UTF-16), or a project.faf, soul.fafm, MEMORY.md, `.faf-dna` or `.grok/config.toml` with a cp1252 `é` in it, was read with those bytes turned into U+FFFD, and the next write put U+FFFD where the user's bytes had been. faf now reads these files strictly as UTF-8. Anything else is refused with "<file> is not UTF-8 — faf left it unchanged", and the file stays byte for byte. A UTF-8 BOM works as before.
- **faf no longer rewrites a `.faf-dna` it cannot prove it wrote.** `faf auto` and `faf refresh` rewrote any `.faf-dna` in faf's shape, so a 20-digit number was rounded, keys that JSON reorders were reordered, a repeated key lost its first value, `\u` escapes were decoded, and a note on the peak milestone went with it. Growth is now added only when the file's text is exactly what faf itself writes for it (2-space JSON and a final newline), no key of yours sits in an entry faf replaces, and the peak and current milestones carry faf's own label and emoji (a peak you relabelled "Launch day" is your text, and stays). Any other `.faf-dna` is read, left byte for byte and gets no growth, and `faf auto`, `faf refresh` and `faf dna` say why in one line.
- **`faf memory convert` no longer replaces a soul that is already there.** It wrote a whole new soul.fafm over the existing one, so a hand-kept index, facts and comments were gone. It now refuses when the output exists, as `faf init` does: "soul.fafm already exists. Use --force to overwrite." Pass `--force` to convert over it.
- **faf no longer writes over an edit made after it read the file.** An edit to soul.fafm while a soul was loaded, to project.faf while `faf auto` was detecting, or another `faf` process's growth in `.faf-dna` was lost to the later write. The writers for CLAUDE.md and the other block files, MEMORY.md, project.faf, soul.fafm and `.faf-dna` now read the file again just before the rename. If it changed, nothing is written and the error says "<file> changed on disk while faf was writing — not written; original kept". A file that appeared while faf was creating one is not written over either. Every write also checks the file's mode just before the rename: a file you made read-only, or gave other permissions, while faf was writing is not replaced (the rename used to bring the old mode back), with the same one-line refusal.

- **A marker example in a code block or an HTML comment is never taken for faf's block.** faf read fences with a plain on/off toggle, so a ```` fence around a ``` line, a `~~~` fence around one, a ``` line with a language after it, a ``` indented four spaces, or a fence never closed made it lose track, take a marker example inside the fence for the block and replace the text around it. Fences are now read the way CommonMark reads them: the closing fence uses the same character, is at least as long, has nothing after it and is indented at most 3 spaces (from the list item's content column, inside a list item), and a fence that is never closed runs to the end of the file. Text inside `<pre>` (and `<script>`, `<style>`, `<textarea>`) blocks and multi-line HTML comments is skipped the same way; faf's own one-line marker comments still count as markers. A start marker with no end marker outside such a region means the file is yours: faf puts its block on top and keeps every byte. This covers all eight block writers (CLAUDE.md, AGENTS.md, GEMINI.md, `.cursorrules`, the Copilot file, MEMORY.md, `llms.txt` and `injectFafBlock`) and claude-faf-mcp's earlier section in Claude Code's MEMORY.md, whose closing line quoted in a later fenced note is no longer taken for the end of the section. When a `.faf` value would leave a fence open inside faf's own block, faf closes it at the end of the block (or, when no closing line works in both of the readings described below, puts `> ` in front of each line of its block), so the block is always found again.
- **Editing project.faf no longer replaces an alias with a stale copy.** `faf edit`, `faf go`, `faf sync --direction pull` and any library call to `writeFaf` with full data (faf-mcp and claude-faf-mcp do this) wrote back every value they had read. With `goal: &g Old goal` and `summary: *g # follows goal`, `faf edit project.goal "New goal"` wrote `summary: Old goal` over the alias and lost its comment. `writeFaf` now compares the data with what the file itself holds and writes only the values that changed, so a value the data only repeats is never written. The alias stays and now reads as the new goal.
- **A comment on a value faf fills in or lifts to a section is kept.** `faf auto` turned `project: demo # note` into `project: { name: demo, … }` and filled `stack: # decide after ADR-7`, and both lost the comment. The comment now stays as a comment line above the new entries, or on the same line when the new value is empty. A value replaced by a different one keeps its line comment. The same holds for soul.fafm: etching into an empty `memory: # note` or `facts: # note` keeps the note above the first fact.
- **A save that changes no data writes nothing, and a `...` at the end of the file stays.** A project.faf or soul.fafm that ended with the YAML `...` marker was rewritten on every save, even one that changed nothing, and lost the `...` and its own spacing. Now a change that leaves the data as it was writes nothing, even when faf's own layout of the file would differ, and a real change keeps the `%YAML` line, `---` and `...` where they were.

- **Every writer goes through the same safe write — there are no exceptions left.** After the round above, `faf refresh` and `faf compile` (the `.fafb`), `faf server-card` (`server.json`), `faf taf` (`taf.yml` and `--output`), `faf diff --install-driver` (`.gitattributes`), `faf go` (its session file), `faf bench` (its state file), `faf hooks`, `faf demo`, `faf git` and `faf clear` (their temp folders) and the star nudge still wrote with a plain file write. A `project.fafb` linked to `~/.zshrc` had the binary written into the shell config, a `server.json` linked to `~/secret.json` was read and rewritten, a dangling `taf.yml` or `project.fafb` link created the file at its far end, and `.gitattributes` linked to `~/.zshrc` got a line appended to it. They now use the same link rules, atomic write and compare-before-rename as every other writer, with the project (or the file's own folder, for a path you name) as the boundary: the file outside stays byte for byte and the refusal is one line.
- **faf never creates a folder through a link.** `.github`, `.github/workflows`, `.grok` and `.well-known` are made one folder at a time inside the project; a folder on the way that is a link leading out of the project (or a dangling one, or anything inside `.git`) is refused, so `.github → ~/elsewhere` can no longer make faf create `~/elsewhere/workflows`. Claude Code's memory folder and a `faf export --output` folder, which are the write's own root, are made as named.
- **A build check keeps it that way.** The trust-seal test now fails if any file in `src/` calls a filesystem API that writes, appends, renames, deletes, makes a folder, links or changes a mode anywhere but `src/core/safe-write.ts`. There are no named exceptions.
- **`faf hooks` touches only its own section of your pre-commit hook.** It found its section with a plain text search, so a hook that quoted the marker text on a line of its own script, or had a start marker with no end, had the text in between replaced. Markers now count only as whole lines, one start before one end; a hook whose markers are anything else is refused in one line and left as it is. Install adds faf's section to a hook it did not write, or updates the section it did; uninstall removes only that section. A hooks folder linked outside the repo (often shared by other repos) is written only to create a new hook or update faf's own section; a hook there that faf did not write is refused. A pre-commit that is a link out of the hooks folder is refused. A missing hooks folder is no longer created inside `.git` — faf says to create it. The hook keeps its mode (execute is added only where read is set). The write goes through safe-write's one narrow allowance for a hook file directly in the repo's hooks folder; everything else in `.git` is still refused.
- **A whole file faf renders replaces only a file faf wrote, byte for byte.** `project.html` (`faf show`, `faf export --html`), the Server Card (`faf export --card`, `faf cards`), the A2A card (`faf cards`), a `.fafb` (`faf compile`, `faf refresh`), a `faf taf --output` snapshot and `faf server-card --out` used to replace any file at that name, including one behind a same-name link (`project.html → docs/project.html`). A mark alone is not enough either: it shows faf wrote the file once, and a hand edit made after that (a `tools` list or a `remotes` endpoint added to the card, a section added to the page, a note added to a snapshot) was lost on the next write. Now every one of these files except the `.fafb` carries faf's render hash, and faf replaces it only while it is byte for byte what faf last wrote (see Changed). A file faf did not write is refused in one line ("… has no <mark>, so faf did not write it — faf left it unchanged. Use --force to replace it."), a file edited since faf wrote it is refused in one line ("<file> was edited since faf wrote it — faf left it unchanged. Use --force to replace it."), and either is kept byte for byte; `--force` replaces it, as `faf init --force` does. A `.fafb` is a compiled binary: it is replaced only when it starts with the FAFB header, and `faf refresh` leaves any other `.fafb` as it is, says so in one line, and carries on.
- **`faf server-card` edits server.json in place.** It rebuilt the file from parsed JSON in its own key order, so a 20-digit `buildId` was rounded, key order and a one-line `packages` array were re-laid out, a hand `title` was deleted when project.faf had none, and any other key under the registry's publisher-provided `_meta` was dropped. Now only faf's identity changes, as text: the value of `name`, of `title` (only when project.faf has one — the file's own title is kept otherwise), of `version` (with `--set-version`), and the keys of faf's context-block, each value token replaced or the key added on a line of its own. No field is deleted, and every other byte stays. A server.json that cannot be edited that way (not valid JSON, a repeated key, a `_meta` that is not an object) is refused in one line. A run that changes nothing writes nothing, and `--check` prints the edited file. `faf cards --target registry` makes the same edit.
- **`faf server-card` and `faf cards` no longer fail with "context block drifted".** With no timestamp in project.faf or in the file, each card read the clock on its own, and a run that crossed a millisecond threw. One timestamp is now used for the whole run.
- **A soul.fafm or `.faf-dna` that appeared meanwhile is not written over.** A soul made with no file (`faf memory etch` with no soul yet, `faf memory convert`) wrote over a soul.fafm that appeared after faf found none, and `FafDNAManager.birth` wrote over a `.faf-dna` that appeared after the manager found none. Both now refuse in one line, and the file stays: a soul.fafm already there when the save starts gets "<file> already exists — not written; pass { replace: true } (faf memory convert --force) to replace it", and one that appears while faf is writing (or a `.faf-dna`) gets the "changed on disk" line. `faf memory convert --force` and `faf init --force` still replace on purpose. A soul saved to a path it was not loaded from does not write over a file there either.
- **`faf go --resume` works.** Quitting the interview saved the session and then deleted it at the end of the same run, so there was nothing to resume. The session is now kept on quit and removed when the interview is finished — only when it is the session faf wrote.
- **The CLI says what faf did not do.** A refused write that faf had read first (`faf auto` with `project.faf → config/team.faf`: the read is allowed, the write is not) said "Nothing was read from or written to it."; it now says "Nothing was written to it." A refusal before any read or write keeps the longer line. A refusal inside `faf go` or `faf ai` is now one line too, not a stack trace, and so is a write that failed with the file kept ("<file>: not written; original kept (EACCES)" for a read-only file, a full disk or a quota), in every command and in `faf hooks --install` over a read-only pre-commit hook.
- **`.grok/config.toml` is not written over an edit made meanwhile.** The grok writer now passes what it read (or that there was no file) to the compare-before-rename check, like the other writers.

- **A marker example inside a list item, or inside any kind of raw HTML block, is never taken for faf's block.** The round above still read list items wrong: a tab after a list marker counted as one column, a fence opened in a list item closed at the wrong column or never ended when the item did, and `<?php … ?>`, `<![CDATA[ … ]]>` and a stray ``` inside `<details>` were not skipped. A fenced marker example after them was taken for the block and replaced. faf now reads every block file two ways: as CommonMark reads it, list items and block quotes included (a list item's lines sit at its content column, a tab after the marker advances to the next multiple of 4, a fence or `<pre>` opened in an item closes at that column and ends when the item does, and no fence opens inside an HTML block such as `<div>` or `<details>` until a blank line), and plainly, every line at column 0. A block is faf's only when both readings find the same start and end lines. When they disagree, the file is yours: faf puts its block on top and keeps every byte. This covers all eight block writers and Claude Code's MEMORY.md.
- **Two start markers before an end marker: the text between them is kept.** faf took the first start and the first end, so a start line you typed above faf's block made everything between the two starts part of the block, and the next write replaced it. The block is now the last start before the first end. faf's own body never holds a start line at column 0, so nothing of faf's sits between the two.
- **A marker line must be exactly the marker.** A `<!-- faf:start -->` or `<!-- faf:end -->` line with a space or a tab after it counted as faf's marker, so text between two such hand-typed lines was replaced. faf never writes whitespace after a marker, so such a line is now text: a file whose only markers look like that is yours, and faf puts its block on top. A CRLF line end still matches.
- **Editing a value with an anchor or a tag changes only that value.** `faf edit project.goal` on `goal: &g Old goal` (or on any value with an anchor like `&g` or a tag like `!!str`) rewrote the whole file: text wrapped by hand was joined onto one line, comment alignment and a flow list's spacing were lost, and `summary: *g   # …` lost its spacing. The anchor and the tag now stay where they are and only the value text changes.
- **An alias is never replaced.** With `stack: *base` (stack follows `defaults: &base`), `faf auto` filled a stack slot by replacing `*base` with a copy of the map: the alias, its comment and the link to `defaults` were gone. faf now leaves every alias as written. `faf auto` fills nothing under it and prints one line ("stack is an alias (*base) — faf left it as written"); `faf edit` of an alias key refuses with that line and exit 1; `faf go`, `faf migrate`, `faf recover`, `faf conductor` and `faf sync --direction pull` print it when a change they carry falls on an alias. A change to the value an alias points to still lands, and the alias follows it.
- **`faf sync` and `faf export` say when an older faf block sits in a code fence.** When faf puts its block on top of a file whose older faf block is inside a code fence, a `<pre>` block or an HTML comment (so faf cannot take it as its own), it now prints one line: "<file>: faf's block is now on top; an older faf block below sits inside a code fence and is left as you had it — delete it by hand if you no longer want it." (The line names the region the older block sits in: a code fence, a `<pre>` or other raw HTML block, or an HTML comment.) Tri-sync puts the same line in MEMORY.md's warnings.
- **The lint gate lints every file.** `npm run lint` ran `eslint src/**/*.ts` unquoted, so the shell expanded the glob two folders deep and `src/cli.ts` and `src/index.ts` were never linted. The glob is now quoted.
- **`npm run check:engines` reads `node-version-file:`.** A workflow step with `node-version-file: .nvmrc` runs the Node that file names, and the check never read it, so a `.nvmrc` of 20 under a floor of 22 passed. The check now takes the named file's first line as the version, and fails when it cannot read the file or the line is not a version number.
- **`faf cards --target catalog` never touches a row that is not faf's.** It matched the catalog's rows by type and URL, so a row of yours for another agent's A2A card (same type, an `agent-card.json` URL) had its URL and date replaced by faf's. Now faf updates only the row whose identifier is exactly faf's (`urn:air:<host>:a2a:<name>` and `urn:air:<host>:agent:<name>`) — only its url, type and updatedAt — and appends its row otherwise. The catalog is edited as text: every other row, its layout and every other byte stays, and a catalog laid out by hand is no longer refused. `upsertCatalog` matches by exact identifier too.
- **`faf compile` never writes the `.fafb` over its source.** With a file whose name did not end in a lowercase `.faf` (`project.FAF`, `notes.fafm`), the default output was the source's own path, and `--force` (meant for the output) replaced the source with binary. The output is now the name with its `.faf` ending (any case) turned into `.fafb`, or `.fafb` added to any other name (`notes.fafm` → `notes.fafm.fafb`), and an output that is the source itself (`--output project.faf`, a case variant or a link to it) is refused in one line, `--force` included.
- **`faf diff --install-driver` and `--uninstall-driver` touch only faf's own driver.** Install set `diff.faf.command` over a value of your own, uninstall removed any value, and a key with two values stopped install with a git error and a stack trace. faf now reads `git config --get-all diff.faf.command` first: install sets it only when it is unset or already faf's value (`faf-cli diff-driver`), and uninstall unsets it only when its one value is exactly that. Any other value (or more than one) is left alone with one line and exit 1, and install then leaves `.gitattributes` as it is too. These two commands, and `faf git`'s clone into faf's own temp folder, are the only writes faf makes through a subprocess, which the trust-seal test cannot see; their effects follow the same owner rule, and the trust-seal notes say so.
- **`faf recover` reads CLAUDE.md, AGENTS.md and GEMINI.md the way faf reads project context.** It read them with a plain read, so an `AGENTS.md` linked to a file outside the project (`~/.aws/credentials`) had its first heading written into project.faf as the project name, and a cp1252 `é` became U+FFFD. Each is now resolved inside the project (a link must stay in the project and end at a `.faf` or `.fafm` file) and read strictly as UTF-8; a refused file is skipped with its one-line reason. When every source is refused, nothing is written and the exit code is 1.
- **`faf clear` removes only temp folders faf made.** It removed every `faf-git-*` folder in the temp folder, so a folder of yours named `faf-git-my-notes` was deleted with what was in it. A folder is now removed only when its name is exactly what faf's temp folders get (`faf-git-` and six letters or digits) and it holds the marker file faf writes into each temp folder it makes (`.faf-temp`). `faf git` now clones into a `repo` folder inside its temp folder, beside the marker.
- **`faf server-card` and `faf cards --target registry` never replace an object or a list with a value.** A server.json whose `title` was an object (`{"en": …, "fr": …}`) or whose `name` was a list had it replaced by faf's plain value. That is refused now in one line, as a value faf would have to go into already was, and the file stays byte for byte.

### Changed
- **One-time step for an existing project.html, Server Card, A2A card, `faf server-card --out` file or `faf taf --output` snapshot: faf now proves it wrote the whole file before it replaces it.** Every one of these files faf writes records a hash of its own content: project.html a `<meta name="faf-render" content="sha256:…">` line (the hash of the page without that line), and the JSON files `_meta["one.faf/render"]` (the hash of the file without that key). The MCP Server Card and the registry server.json schemas leave `_meta` open for namespaced keys, and the registry keeps only its publisher-provided key on publish. The A2A Agent Card has no `_meta` field; the A2A spec says implementations should ignore a field they do not know, and the official Python and Go SDKs do. On the next write faf recomputes the hash. A match means the file is byte for byte what faf last wrote, so faf replaces it; a mismatch means it was edited since, so faf leaves it and says "<file> was edited since faf wrote it — faf left it unchanged. Use --force to replace it." A file written by an earlier faf has faf's mark but no hash. If it is exactly faf's render of the current project.faf, faf takes it as its own and leaves it as it is. Otherwise faf refuses it once: "<file> has no faf render hash (written before 7.13), so faf cannot tell whether it was edited — faf left it unchanged. Use --force once to replace it; after that faf recognises its own output." Check the file for hand edits you want to keep, then run the same command with `--force` once. A `faf export` or `faf cards` run that refuses one file still writes the others, prints the one-line refusal for each refused file, and exits 1. A `.fafb` keeps its FAFB header check.
- **`writeFaf` merges into an existing file by default (library users).** Since this release, `writeFaf(path, data)` on a project.faf that exists compares `data` with what the file holds and writes only the values that changed: keys the file has and `data` leaves out are kept, a value `data` only repeats (an alias's value, say) is left as the file has it, comments and formatting stay, and a file that is not valid YAML is refused. Pass `{ replace: true }` for the 7.12 behaviour, a fresh file from `data`; `faf init --force` and `faf git --force` do. `writeFaf` also returns a boolean now — `false` means nothing was written — so a stub typed as `typeof writeFaf` must return one.
- **A file that starts with faf's old two-line stamp is prefixed, never reclaimed.** A CLAUDE.md, AGENTS.md, GEMINI.md, `.cursorrules`, Copilot file or `llms.txt` whose first line is `<!-- faf: … -->` but has no block markers used to be replaced whole. faf cannot prove it wrote that text, so its block now goes on top and the old text stays below it. `faf sync` and `faf export` say so in one line: "<file>: faf's block is now on top; the old faf text below it is left as you had it — delete it by hand if you no longer want it."
- **Every file faf rewrites gets a new inode.** faf writes a temp file and renames it over the original (see Fixed), so the rewritten file is a new file on disk. Another hard link to the old file keeps the old bytes and no longer sees faf's changes. The file's mode is kept; its inode number is new.
- **Node 22 is the floor.** `engines.node` is now `>=22.0.0`. It said 18, while commander 14 and open 11 need Node 20 or later, so every package built on faf-cli inherited a false floor. CI smoke-tests Node 22 and 24, the release matrix tests Node 22 and 24, and `npm run check:engines` fails CI (Code Quality) and `prepublishOnly` when any Node version in any workflow (every `node:` / `node-version:` value, as a list, a single value or an `env` reference, and the version in the file a `node-version-file:` names) is below the floor, or when the lowest one is not the floor. It used to read only the first matrix in two files, so a second job running Node 20 passed.
- **The whole-file writers refuse a file faf cannot prove it wrote (library users).** `writeProjectHtml`, `writeServerCard` and the `faf cards` JSON writer throw a `SafePathError` with the new reason `not-owned` over a file without faf's mark, a file edited since faf wrote it, or a file from before 7.13 that is not exactly faf's render (see the one-time step at the top of this section), and change nothing. Pass `{ force: true }` (`writeProjectHtml(dir, data, result, fafPath, { force: true })`, `writeServerCard(dir, data, opts, { force: true })`) to replace it on purpose. What they write now carries faf's render hash.
- **`FafmSoul.save` of a new soul does not write over a file that is there (library users).** A soul made in memory (or saved to a path it was not loaded from) refuses a file already at the path; pass `{ replace: true }` to write over it on purpose. `FafDNAManager.birth` refuses a `.faf-dna` that appeared after the manager found none; over one that was there when it looked, it still starts a fresh lineage.
- **`faf server-card` keeps the file's own layout.** Its output is no longer in faf's fixed key order: a key faf adds goes on a line of its own (a missing `title` after `name`, a missing `_meta` before the last key), and everything else stays where the file has it. `--check` prints the edited file, so a test that compares it with the live file still holds after a run.
- **`faf hooks --install` no longer creates a missing hooks folder.** faf does not create folders inside `.git`; it says to create `.git/hooks` and re-run.
- **A block after a list-item fence closed at its item's column is prefixed, not updated in place.** When a fence opens right after a list marker (`- ```bash`) and closes two spaces in, at the item's column, CommonMark closes it there; read at column 0, that closing line opens a fence that hides what follows. The two readings disagree, so faf cannot prove the block below is its own: it puts a new block on top, leaves the old one as you had it, and says so in one line. A marker line with a space or tab after it is text now too, so a block marked that way also gets faf's block on top of it.
- **`writeFaf` and `updateFafFile` never replace an alias (library users).** A change that `data` (or `mutate`) makes at an alias path is not written; the alias stays as written and the rest of the change is written. `writeFaf(path, data, { onAliasKept })` calls `onAliasKept({ path, alias })` for each such path, and `updateFafFile` returns them in `keptAliases`. Round 2 replaced an alias key on an explicit change to it.
- **`legacyStampNote` also names an older faf block in a code fence (library users).** Besides the stamp line, it returns the fence line (see Fixed) when faf's block goes on top of a file whose start marker sits in a fence, a `<pre>` block or an HTML comment.

### Removed
- **`faf ai enhance` is retired.** project.faf isn't enhanced: a tech slot holds a repo fact, a 6W holds your own words, and an AI suggestion is neither. `faf ai enhance` now prints one line ("faf ai enhance was retired in 7.13 — project.faf isn't enhanced: faf auto fills tech slots from repo facts, and you write the 6Ws (faf go)."), writes nothing and exits 1. Use `faf auto` to fill tech slots from repo facts and `faf go` to write the 6Ws. `faf ai analyze`, which only reads project.faf and prints suggestions, stays, with the optional `@anthropic-ai/sdk` it uses.

### Note
- An atomic write gives the file a new inode. Another hard link to the file keeps the old bytes, and extended attributes (a Finder comment, `user.*` attributes) are not carried over to the new file. faf keeps the file's mode, and its owner where the system allows.
- Migrating claude-faf-mcp's old tri-sync section in MEMORY.md replaces that whole tool-managed section, from its heading line to its closing line, including any line typed inside it. The notes around the section are kept byte for byte.
- A project.faf written in JSON syntax stays valid after a faf edit, but faf's additions use YAML flow style (unquoted keys and values) rather than JSON. This is cosmetic and is left as it is in this release.
- `--force` replaces a whole file, text and all. For the files faf renders whole (project.html, the Server Card, the A2A card, a `.fafb`, a `faf taf --output` snapshot, and `server.json` with `faf server-card --out`), a file faf cannot prove it wrote — no faf mark, edited since faf wrote it, or from before 7.13 and not exactly faf's render — is refused unless you pass `--force`. With `--force`, faf writes its own render over the whole file, as `faf init --force` does, and that file's own text is gone. Apart from faf's own block and the migration above, `--force` (and `{ force: true }` or `{ replace: true }` in the library) is the only way faf replaces text it did not write, and only when you ask for it.
...
- `resolveInside(dir, name, { read })`, `safeWriteFile(path, content, { root })` and `SafePathError` are exported: the safe resolve and atomic write every faf writer now uses, so MCP servers built on faf-cli can route their own reads and writes through the same rules. `injectFafBlock` takes an optional `{ root }` so a file in a subfolder (`.github/…`) is kept inside the project folder, not just its own folder.
- `updateFafFile(path, mutate)` is exported. It parses a .faf with yaml's `parseDocument`, lets `mutate` change the Document (`setIn`, `deleteIn`, …) and writes back only what changed, through `safeWriteFile`. It skips the write when nothing changed, and returns `{ written, text }` (and `keptAliases`, see below). `writeFaf` uses it for existing files, takes an optional `{ replace: true }` for an explicit overwrite, and returns whether it wrote.
- `isExplicitNone(value)`, `EXPLICIT_NONE` and `SLOTIGNORED` are exported, so consumers treat a typed none the way faf does: a typed none counts as an empty slot, and `fillEmpties` and `updateExistingFaf` replace it only with a repo fact, only in a tech slot, as `faf auto` does.
- `FafmSoul.derivedIndex()` and `FafmSoul.indexIsDerived()` are added. The first returns the index faf would derive from the facts without changing the soul. The second says whether the stored index is exactly that one. The `FafmRecallOptions` type is exported too.
- `FafDNAManager` is exported, with the `FafDNA`, `BirthCertificate`, `VersionEntry` and `Milestone` types. It is the birth / recordGrowth / getJourney API that `faf init`, `faf auto` and `faf dna` use. `isFafShape()` says whether a `.faf-dna` is one faf may add to, or one it only reads.
- `registryTitle(data)` is exported: the registry display title from `project.title`, trimmed, at most 100 characters (undefined otherwise).
- `normalizeGitUrl`, `repoNameFromUrl` and `authorFafFromRepo(dir, { repoUrl })` are exported. They are the parts of `faf git` that run no git: fetch a repo your own way, and `authorFafFromRepo` returns the FafData `faf git` would write for it. Nothing exported runs git.
- `writeClaudeMemory(dir, data)` writes faf's block into the MEMORY.md Claude Code loads for a project and returns what it did (`created`, `updated`, `migrated`, `added` or `unchanged`), whether every byte outside the block is still there (checked by reading the file back), and warnings: the file is past the 200 lines Claude Code loads, or the block went on top of an older faf block that sits in a code fence or comment. Markers match whole lines only. Only a missing file counts as "no file"; any other read error is thrown and nothing is written. claude-faf-mcp's earlier tri-sync section is replaced in place, once, when both its heading line and its closing line are there as whole lines.
- `resolveClaudeMemoryPath(dir)`, `resolveClaudeMemoryDir(dir)`, `claudeProjectRoot(dir)` and `claudeProjectId(root)` follow Claude Code's own rule for that path: the canonical git root (a linked worktree maps to its main checkout; a folder outside git is its own root), every character outside `[a-zA-Z0-9]` turned into `-`, a cut at 200 characters plus a hash for longer ids, and `CLAUDE_CONFIG_DIR`. `renderClaudeMemory(data)` returns the block body, and `claudeMemoryStatus(dir)` reads the same file without writing anything: whether faf's block (or claude-faf-mcp's earlier section) is there, and how many lines are Claude's own.
- `scoreText(result)` is exported: `85%`, or `unknown (—)` for a result marked `unknown`. The `ScoreResult.unknown` flag is documented: render it as unknown, never as a number, and do not seal a receipt for it.
- `safeWriteFile(path, content, { expect })` takes the bytes the caller read (a string or a Buffer; `null` when there was no file) and refuses the write if the file no longer holds them. `readUtf8(path)` is exported: the strict UTF-8 read faf's writers use, so MCP servers built on faf-cli refuse the same files. `SafePathError` has four new reasons: `other-file` (a link to a file with another name), `git` (inside `.git`), `not-utf8` and `changed`.
- `FafDNAManager.readOnlyReason()` says in one line why a `.faf-dna` is read and left as it is, or returns null when faf may add growth to it.
- `faf memory convert --force` converts over an existing soul.fafm.
- The `ProjectHtmlWriteOptions` and `CardWriteOptions` types are exported from the package root: the `{ force }` options `writeProjectHtml` and `writeServerCard` take.
- `safeReplaceOwned(path, content, { owns, mark, force, root, expect })` writes a whole file only over a file whose bytes carry faf's mark (`owns`), else throws `SafePathError` `not-owned`; `makeDirInside(root, dir)` makes a folder inside the project, never through a link out; `safeUnlink(path, { expect })` removes a file only while it still holds the bytes faf wrote, and never a link. The `ReplaceOwnedOptions` type is exported. `SafePathError` has a fifth new reason, `not-owned`, and an `onWrite` flag (true when faf refused at the write itself, having possibly read the file first). `safeWriteFile` takes `mode` (the permission bits for the written file) and `allowGitHooks` (the one exception to the `.git` rule: a hook file directly in the repo's hooks folder, `root` being that folder).
- `patchServerJson(text, { name, title, version, meta })` is exported, with the `ServerJsonIdentity` type: the text-preserving identity edit `faf server-card` makes, for JS build scripts that keep their own server.json.
- `--force` on `faf show`, `faf export`, `faf cards`, `faf server-card` (with `--out`), `faf compile` and `faf taf` (with `--output`) replaces a file faf did not write. `writeProjectHtml` and `writeServerCard` take `{ force }`, and `FafmSoul.save` / `toFile` take `{ replace }`.
- `legacyStampNote(label, text)` and `legacyStampNoteAt(path, label)` are exported from the package root: the one line `faf sync` and `faf export` print when faf's block goes on top of older faf text, so MCP servers built on faf-cli can print the same line.
- `WriteFafOptions.onAliasKept`, `UpdateFafResult.keptAliases` and the `KeptAlias` type (`{ path, alias }`): the alias paths a write left as written.
- `faf score` and `faf auto` print one line for each slot the app-type needs that still holds a typed None / N/A / not applicable: "stack.database says 'None' — this app-type needs it, so it counts as empty until filled." Nothing is written. `faf score --verbose` lists such a slot as empty, never as N/A. `faf go` asks about it like any empty slot, and the question shows your words, so Enter keeps them.
...
### Changed
- **Interop emitter banners now say "authored," not "generated."** FAF *authors* context from a repo's own truth — the banners now say so (#102).

_Note: npm goes 7.1.0 → 7.1.2. v7.1.1 (the `--stdout` fix, #100) was tagged and GitHub-released but never published to npm; 7.1.2 carries it forward._
