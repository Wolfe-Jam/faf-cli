...
- **`faf auto` fills a slot under the name your file uses, from the repo's fact.** A file with `stack.db: None` got a second key, `stack.database`, beside it; faf now adds no such twin, and a repo fact (a `postgres` image in docker-compose) fills `stack.db` itself. A slot your app-type uses got `slotignored` when detection read the repo as another type; it now gets the repo's fact (`runtime: Go` from go.mod), or keeps what it had.
