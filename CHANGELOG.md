...
- **A whole file faf renders replaces only a file faf wrote.** `project.html` (`faf show`, `faf export --html`), the Server Card (`faf export --card`, `faf cards`), the A2A card (`faf cards`), a `.fafb` (`faf compile`, `faf refresh`), a `faf taf --output` snapshot and `faf server-card --out` used to replace any file at that name, including one behind a same-name link (`project.html → docs/project.html`). Now a file already there is replaced only when it carries faf's own mark — the one faf's 7.12 output already carries, so a file faf wrote before still updates with no flag: project.html's `Visual render of project.faf` description line, the Server Card's `_meta["one.faf/context"]`, the A2A card's FAF context extension, a `.fafb`'s FAFB header, a snapshot's `taf_version`, a server.json's faf identity. Any other file is refused in one line ("… has no <mark>, so faf did not write it — faf left it unchanged. Use --force to replace it.") and kept byte for byte; `--force` replaces it, as `faf init --force` does. `faf refresh` leaves such a `.fafb` as it is, says so in one line, and carries on. The `faf cards` catalog, which faf shares with you, is rewritten only when its text is exactly faf's JSON layout, so the rewrite loses nothing.
...
- **The CLI says what faf did not do.** A refused write that faf had read first (`faf auto` with `project.faf → config/team.faf`: the read is allowed, the write is not) said "Nothing was read from or written to it."; it now says "Nothing was written to it." A refusal before any read or write keeps the longer line. A refusal inside `faf go` or `faf ai` is now one line too, not a stack trace.
...
- **The whole-file writers refuse a file faf did not write (library users).** `writeProjectHtml`, `writeServerCard` and the `faf cards` JSON writer throw a `SafePathError` with the new reason `not-owned` over a file without faf's mark, and change nothing. Pass `{ force: true }` (`writeProjectHtml(dir, data, result, fafPath, { force: true })`, `writeServerCard(dir, data, opts, { force: true })`) to replace it on purpose. A file they wrote before is replaced as in 7.12.
