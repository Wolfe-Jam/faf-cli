...
- **`faf drift --json`** — structured output for the context-drift check, parity with `faf score --json`: a self-describing `faf_version` / `project` / `source` header spread over the report, raw `*_ms` mtimes and deltas (no `Date.now()` in the payload — the consumer formats its own "5d ago"). The comparison moves to a pure `computeDrift(fafPath, dir?)` in `src/core/drift.ts`, exported from the package index — programmatic consumers (the VS Code extension) import it as a library instead of shelling out. Bare `faf drift` output is unchanged.
