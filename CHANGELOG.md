...
- **A hand-written `None`, `N/A` or `not applicable` is a decision, not a gap.** `faf auto` used to replace `database: None # deliberate: stateless` with a detected PostgreSQL, and `frontend: N/A` with React, while saying existing values were kept. Now no detected value replaces it. When faf writes the slot it records `slotignored`, which means the same thing, and the comment stays on the line. This is also true when the none is under the slot's other name (`stack.db` for `stack.database`), and for a slot that already says `slotignored` under either name. Outside the 33 slots, a none is kept exactly as written.
- **Saving a soul.fafm keeps what the file had.** A save used to rebuild the file. A hand-kept `index` was replaced by generated lines, comments were lost, `version: 2.0` became `1.1`, and a `profile: voice` the file never had was added. `FafmSoul.save` now edits the loaded file in place and writes only what changed: new facts, changed fields and `last_etched`. A save that changes nothing writes nothing.
- **Saving a soul.fafm no longer rebuilds the index unless asked.** `save(path, { reindex: true })` rebuilds it. `faf memory etch` keeps an index in step only when it is exactly the one faf itself derives from the facts; a hand-kept index is left alone.
...
### Changed
- **`writeFaf` merges into an existing file by default (library users).** Since this release, `writeFaf(path, data)` on a project.faf that exists changes only the values that differ from `data`: keys the file has and `data` leaves out are kept, comments and formatting stay, and a file that is not valid YAML is refused. Pass `{ replace: true }` for the 7.12 behaviour, a fresh file from `data`; `faf init --force` and `faf git --force` do.
- **Node 22 is the floor.** `engines.node` is now `>=22.0.0`. It said 18, while commander 14 and open 11 need Node 20 or later, so every package built on faf-cli inherited a false floor. CI smoke-tests Node 22 and 24, the release matrix tests Node 22 and 24, and `npm run check:engines` fails CI (Code Quality) and `prepublishOnly` when the floor and either matrix disagree.
