...
- **One-time step for an existing project.html, Server Card, A2A card, `faf server-card --out` file or `faf taf --output` snapshot: faf replaces a whole file only when it is exactly what faf last wrote.** Each of these files now carries a hash of faf's own render: project.html a `<meta name="faf-render" content="sha256:…">` line (the hash of the page without that line), the JSON files `_meta["one.faf/render"]` (the hash of the file without that key). The MCP Server Card and registry server.json schemas leave `_meta` open for namespaced keys (the registry keeps only its publisher-provided key on publish). The A2A Agent Card has no `_meta` field, and the A2A spec says a reader should ignore a field it does not know, as the official Python and Go SDKs do. On each write faf recomputes the hash:
  ...
  - the file has faf's mark but no hash (written before 7.13): if it is exactly faf's render of the current project.faf, faf leaves it as it is; otherwise faf refuses it once: "<file> has no faf render hash (written before 7.13), so faf cannot tell whether it was edited — faf left it unchanged. Use --force once to replace it; after that faf recognises its own output.";
  - the file has no faf mark at all: "… has no <mark>, so faf did not write it — faf left it unchanged. Use --force to replace it."

  Check a refused file for hand edits you want to keep, then run the same command once with `--force`, which writes faf's render over the whole file, text and all. A `faf export` or `faf cards` run that refuses one file still writes the others, prints one line per refused file and exits 1. A file git checked out with CRLF line ends (`core.autocrlf`) is still faf's when that is its only difference: faf reads its CRLF line ends as LF to check the hash, and writes the new render with CRLF. In the library, `writeProjectHtml`, `writeServerCard` and the `faf cards` writer throw a `SafePathError` with reason `not-owned` in these cases and change nothing; pass `{ force: true }` to replace the file on purpose. A `.fafb` is a compiled binary: faf replaces it only when it starts with the FAFB header.
- **Words typed into a slot count as an empty slot.** `None`, `N/A`, `not applicable`, `unknown`, `"null"` and the other placeholder words, in any case, score 0 until the slot is filled; the app-type alone decides which slots count. `faf auto` handles them by slot:
  ...
  - a tech slot the app-type leaves out (`database` in a library, say): the app-type's decision is the fact, so `faf auto` writes `slotignored` (shown as N/A) in place of the words, comment kept — so such a project can reach 100, because the score does not read the app-type and counts a slot as empty until it holds a value or `slotignored`. A real value there is kept. The `library` type detection falls back to when the repo has no classifying signal decides nothing while its line carries faf's note (`type: library # found: no classifying signals — fallback`); a type you set does;
  - a 6W (`human_context.*`): the words are yours. `faf auto` never replaces them; `faf go` asks, shows your words, and Enter keeps them.

  `faf auto` writes `slotignored` only where the file's own app-type leaves a slot out — never into a slot that type uses because detection read the repo as another type. `faf score` prints a line for each scored slot that holds typed words: "stack.hosting says 'unknown' — this app-type needs it, so it counts as empty until filled." or "stack.database says 'None' — this app-type doesn't use it; faf auto marks it slotignored (N/A)." `faf auto` prints the first kind for the slots it could not fill. Nothing is written for a hint, and `faf score --verbose` shows such a slot as empty, never as N/A.
...
- **When faf cannot read a block file for sure, its block goes on top and every byte stays.** That now covers:
  - a CLAUDE.md, AGENTS.md, GEMINI.md, `.cursorrules`, Copilot file or `llms.txt` that starts with faf's old two-line stamp (`<!-- faf: … -->`) but has no block markers — it used to be replaced whole;
  ...
  The old text stays below the new block — delete it by hand if you no longer want it. For the old stamp, a block the two readings disagree on and a block past deep nesting, `faf sync` and `faf export` say so in one line.
- **A new soul never writes over a soul.fafm that is there (library users).** `FafmSoul.save` of a soul made in memory, or saved to a path it was not loaded from, refuses a file already at the path: "<file> already exists — not written; pass { replace: true } (faf memory convert --force) to replace it". Pass `{ replace: true }` to write over it. `FafDNAManager.birth` refuses a `.faf-dna` that appeared after the manager found none; over one that was there when it looked, it starts a fresh lineage, as `faf init --force` does.
- **`faf server-card` keeps the file's own layout.** Its output is no longer in faf's fixed key order: a key faf adds goes on a line of its own (a missing `title` after `name`, a missing `_meta` before the last key), and everything else stays where the file has it. `--check` prints the edited file.
...
- **Node 22 is the floor.** `engines.node` is `>=22.0.0`. It said 18, while commander 14 and open 11 need Node 20 or later, so every package built on faf-cli inherited a false floor. CI and the release matrix test Node 22 and 24, and `npm run check:engines` fails CI and `prepublishOnly` when any Node version in any workflow — every `node:` / `node-version:` value (a list, a single value or an `env` reference) and the file a `node-version-file:` names — is below the floor, or when the lowest one is not the floor.

### Fixed
- **Every writer is non-destructive, with no exceptions.** No faf writer replaces, reorders, truncates or deletes text it cannot prove it wrote. It touches only its own block or its own section; when it cannot prove that, it refuses in one line and the file stays byte for byte. Every write in `src/` goes through one module, `src/core/safe-write.ts`, and a build check (the trust-seal test, `tests/write-guard.test.ts`) fails if any other file calls a filesystem API that writes, appends, renames, deletes, makes a folder, links or changes a mode. `faf diff --install-driver` (git config) and `faf git`'s clone into faf's own temp folder write through a subprocess, which the trust-seal test cannot see; their effects follow the same rule. `faf diff --uninstall-driver` edits the repo's git config file through `src/core/safe-write.ts` like every other writer.
- **A file without faf's block markers is never taken over, whatever its first or last line says.** A CLAUDE.md, AGENTS.md, GEMINI.md, `.cursorrules` or Copilot file that started with faf's stamp was erased whole. faf now puts its block on top; the only text it replaces is what sits between its own markers. A file that starts with a BOM keeps it at byte 0. MEMORY.md (tri-sync) is written through the same managed block instead of being replaced whole.
- **A marker example is never taken for faf's block.** faf read fences with a plain on/off toggle, so a ```` fence around a ``` line, a `~~~` fence, a ``` line with a language after it, an indented ```, a fence never closed, a fence in a list item, `<pre>`, `<?php … ?>`, `<![CDATA[ … ]]>` or a multi-line HTML comment made it take a marker example inside for the block and replace the text around it. faf now reads every block file two ways — as CommonMark reads it (list items and block quotes included) and plainly, every line at column 0 — and takes a block as its own only when both readings find the same start and end lines, outside every code block, raw HTML block and comment. A marker line must be exactly the marker. With two start lines before an end line, the block is the last start before the first end, so a start line you typed above faf's block no longer pulls your text into it. When a `.faf` value would leave a fence open inside faf's own block, faf closes it at the end of the block (or quotes the block's lines with `> `), so the block is always found again. This covers CLAUDE.md, AGENTS.md, GEMINI.md, `.cursorrules`, the Copilot file, MEMORY.md, `llms.txt`, `injectFafBlock` and claude-faf-mcp's earlier section in Claude Code's MEMORY.md.
- **Deep nesting no longer stalls a write.** Reading a block file cost time in proportion to the square of its nesting depth, or worse: with a list nested 1,200 levels deep, placing faf's block took 14 seconds. Reading is now linear in the file's length at any depth.
- **faf's own block never nests too deep for its next run.** A `.faf` value that nests more than 100 block quotes, lists and list items (a 101-deep `>>>…` line in project.goal, or a list nested 52 levels) made `faf export` and `faf sync` stop with a stack trace. In its own block faf now escapes, with a backslash (`\>`, `\-`, `1\.`), each marker that would open more than 16 of them, so the text reads the same and the next run finds the block. A block faf still cannot place where its next run finds it again is refused in one line — "<file>: faf could not place its block where the next run finds it again — faf left it unchanged" — the file stays byte for byte, and `faf export` writes its other files and exits 1. `injectFafBlock` and `writeClaudeMemory` throw a `SafePathError` with reason `unplaceable`.
- **faf never reads or writes through a link that leads out of the project.** With `project.faf → ~/.aws/credentials` in a cloned repo, the secret was read as project context; `CLAUDE.md → ~/.zshrc` got faf's block written into the shell config. faf now refuses a link that leaves the project, and a dangling link, before it reads or writes anything. Inside the project, faf follows a link only to the same kind of file: one with the same name (`CLAUDE.md → docs/CLAUDE.md`), or another AI context file (`CLAUDE.md → AGENTS.md`); `CLAUDE.md → README.md`, `CLAUDE.md → package.json` and `project.html → .git/config` are refused. When faf reads project context through a link, the link must end at a `.faf` or `.fafm` file, so `project.faf → .env` is refused too; a read of one AI context file through a link to another (`CLAUDE.md → AGENTS.md`, as `faf recover` makes) follows the writers' rule. Anything inside `.git` is refused, always, with two allowances: faf's section of a hook file directly in the repo's hooks folder, and the driver section faf wrote in the repo's own git config file (`faf diff --uninstall-driver`). faf makes `.github`, `.github/workflows`, `.grok` and `.well-known` one folder at a time inside the project and never creates a folder through a link; Claude Code's memory folder and a `faf export --output` folder, which are the write's own root, are made as named. The refusal is one line that names the link and its target.
...
- **faf never writes over an edit made after it read the file.** An edit to soul.fafm while a soul was loaded, to project.faf while `faf auto` was detecting, or another `faf` process's growth in `.faf-dna` was lost to the later write. Every writer now reads the file again just before the rename; if its bytes or its mode changed, nothing is written: "<file> changed on disk while faf was writing — not written; original kept". A file that appeared while faf was creating one is not written over either.
...
- **`faf auto` reads a project.faf that starts with a BOM.** It wrote the file, then exited 1 ("more than one document") while scoring it, and so did `faf score` on such a file with more than one top-level key. The score now leaves a leading BOM out (`kernel.score` and `kernel.scoreEnterprise` too); the file keeps its BOM.
...
- **`faf memory convert` no longer replaces a soul that is already there.** It refuses when the output exists — "soul.fafm already exists. Use --force to overwrite." — as `faf init` does.
...
- **`faf server-card` edits server.json in place.** It rebuilt the file from parsed JSON in its own key order, so a 20-digit `buildId` was rounded, a one-line `packages` array was re-laid out, a hand `title` was deleted when project.faf had none, and other keys under the registry's publisher-provided `_meta` were dropped. Now only faf's identity changes, as text: `name`, `title` (only when project.faf has one), `version` (with `--set-version`) and the keys of faf's context-block. No field is deleted. A server.json faf cannot edit that way — not valid JSON, a repeated key, a `_meta` that is not an object, or an object or list where faf would put a plain value (a `title` of `{"en": …}`) — is refused in one line. `faf cards --target registry` makes the same edit.
...
- **`faf diff --install-driver` and `--uninstall-driver` touch only faf's own driver.** Install set `diff.faf.command` over a value of yours and uninstall removed any value; `git config --unset` also took your comment on faf's line, or the key written on the `[diff "faf"]` header line, with it. faf now reads `git config --get-all diff.faf.command` first. Install sets it only when it is unset or already `faf-cli diff-driver`, and only when no other `diff.faf.*` setting of yours (a `textconv`) is there. Uninstall removes only the section install writes — a `[diff "faf"]` line and a tab-indented `command = faf-cli diff-driver` line, nothing else in it — as a text edit of the repo's own config file, checked with git's own reading of the result. Anything else — a comment on either line or in the section, a setting of yours in it, another spelling, the key on the header line, faf's value set outside the repo — is left alone with one line and exit 1, and `.gitattributes` is left as it is too.
- **`faf hooks` touches only its own section of your pre-commit hook.** A hook that quoted the marker text, or had a start marker with no end, had the text in between replaced. Markers now count only as whole lines, one start before one end; any other shape is refused in one line. Install adds faf's section to a hook it did not write, or updates the section it did; uninstall removes only that section; the hook keeps its mode (execute is added only where read is set). In a hooks folder linked outside the repo, faf only creates a new hook or updates its own section, and a pre-commit that is a link out of the hooks folder is refused.
- **`faf recover` reads CLAUDE.md, AGENTS.md and GEMINI.md the way faf reads project context.** An `AGENTS.md` linked to a file outside the project had its first heading written into project.faf as the project name, and a cp1252 `é` became U+FFFD. A refused source is now skipped with its one-line reason; when every source is refused, nothing is written and the exit code is 1. A link from one AI context file to another (`CLAUDE.md → AGENTS.md`) is read, the rule the writers use; a read through any other link (`CLAUDE.md → README.md`) is still refused.
- **`faf clear` removes only temp folders faf made.** It removed every `faf-git-*` folder in the temp folder, so a folder of yours named `faf-git-my-notes` went with everything in it. A folder is removed now only when its name is exactly faf's (`faf-git-` and six letters or digits) and it holds the `.faf-temp` marker faf writes into each temp folder it makes. `faf git` clones into a `repo` folder beside that marker.
- **`.grok/config.toml` is not written over an edit made meanwhile** — the grok writer checks what it read, like the other writers.
- **The CLI says what faf did not do, in one line.** A refused write that faf had read first (`faf auto` with `project.faf → config/team.faf`) says "Nothing was written to it."; a refusal before any read or write keeps "Nothing was read from or written to it." A refusal or a "not written; original kept" failure prints one line in every command, including `faf go`, `faf ai` and `faf hooks --install` over a read-only hook — never a stack trace.
...
- **`faf sync` and its docs say which way it writes.** The help said "mtime auto-direction"; since #63 `faf sync` has always written CLAUDE.md from `.faf`, never the other way. The help, the README, docs/GUIDE.md, docs/PRO.md and docs/SYNC.md now say so: `faf sync` writes `.faf` → CLAUDE.md, tri-sync writes `.faf` → MEMORY.md, and `faf sync --direction pull` is the only way back into `.faf`, only at ✪ Trophy.
...
- **`npm run check:engines` reads `node-version-file:`.** A workflow step with `node-version-file: .nvmrc` runs the Node that file names, and the check never read it, so a `.nvmrc` below the floor passed. It now reads the file's first line as the version, and fails when it cannot read the file or the line is not a version number.
...
- **CLI:** `--force` on `faf show`, `faf export`, `faf cards`, `faf server-card` (with `--out`), `faf compile` and `faf taf` (with `--output`) replaces a file faf cannot prove it wrote; `faf memory convert --force` converts over an existing soul.fafm.
