...
### Added
- **`faf cards`** — one projector. `.faf` + `.fafa` → A2A Agent Card · MCP Server Card · registry `server.json` · AI Catalog. Same `fafContextBlock()` on every door. `faf server-card` now wraps the registry target.
