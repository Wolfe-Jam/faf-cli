...
- When `faf auto` (or another faf edit) changes a flow-style list that holds a comment — `tags: [a, # note` on one line, `b]` on the next — it may re-lay out that list, with the brackets and each item on lines of their own. Every item and every comment is kept.
