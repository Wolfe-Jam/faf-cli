...
### Changed
- **`writeFaf` merges into an existing file by default (library users).** Since this release, `writeFaf(path, data)` on a project.faf that exists compares `data` with what the file holds and writes only the values that changed: keys the file has and `data` leaves out are kept, a value `data` only repeats (an alias's value, say) is left as the file has it, comments and formatting stay, and a file that is not valid YAML is refused. Pass `{ replace: true }` for the 7.12 behaviour, a fresh file from `data`; `faf init --force` and `faf git --force` do.
...
- **A hand-written None / N/A / not applicable scores as `slotignored`.** faf now keeps those words in the file, and `scoreFafYaml` and the exported `kernel.score` read them at a slot as `slotignored`. A file with such a slot scores a little higher than in 7.12, where the kernel counted it as an empty slot; it scores the same as round 2's `faf auto` output, which wrote `slotignored` in its place.
...
- `updateFafFile(path, mutate)` is exported. It parses a .faf with yaml's `parseDocument`, lets `mutate` change the Document (`setIn`, `deleteIn`, …) and writes back only what changed, through `safeWriteFile`. It skips the write when nothing changed, and returns `{ written, text }`. `writeFaf` uses it for existing files, takes an optional `{ replace: true }` for an explicit overwrite, and returns whether it wrote.
...
- `writeClaudeMemory(dir, data)` writes faf's block into the MEMORY.md Claude Code loads for a project and returns what it did (`created`, `updated`, `migrated`, `added` or `unchanged`), whether every byte outside the block is still there (checked by reading the file back), and a warning when the file is past the 200 lines Claude Code loads. Markers match whole lines only. Only a missing file counts as "no file"; any other read error is thrown and nothing is written. claude-faf-mcp's earlier tri-sync section is replaced in place, once, when both its heading line and its closing line are there as whole lines.
