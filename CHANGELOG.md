...
- **A project.faf that is not valid YAML is one line, never a stack trace.** `faf auto`, `faf export`, `faf sync`, `faf go`, `faf check` and the other commands that read project.faf through faf's .faf reader printed yaml's stack trace for a repeated key; they now print "<file> is not valid YAML (Map keys must be unique, line 8) — faf left it unchanged" and exit 1. `readFaf`, `writeFaf` and `updateFafFile` throw a `SafePathError` with reason `not-yaml` (yaml's error is its `cause`). A project.faf that parses to a scalar or a list (`just a string`) printed a stack trace; it is now one line in every command that reads it — "<file>: a .faf must be a YAML mapping (key: value pairs), but this one is a string ("just a string"). faf left it unchanged — fix it by hand." — and the library throws the same `SafePathError` (`not-yaml`).
- **`faf score`, `faf status`, `faf compile` and `faf refresh` read project.faf before they score or compile it.** They handed its text to the scoring kernel unread, so a repeated key printed the kernel's own error — under Node, an UnhandledPromiseRejection with a stack trace. They now print the same one line as the other commands and exit 1. Text that yaml reads but the kernel cannot (a 30-digit integer, nesting past the kernel's depth limit) is one line too: "<file>: faf's scoring kernel could not read it (<reason>) — faf left it unchanged".
