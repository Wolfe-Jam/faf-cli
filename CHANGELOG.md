...
- **A project.faf that starts with a BOM scores.** `faf score` stopped with "more than one document" on such a file with more than one top-level key. The score now leaves a leading BOM out — in `faf score`, in `faf auto` (which now keeps the BOM when it edits the file) and in the library's `kernel.score` and `kernel.scoreEnterprise` — and the file keeps its BOM.
