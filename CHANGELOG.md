...
- **A hand-written `None`, `N/A` or `not applicable` is a decision, not a gap.** `faf auto` used to replace `database: None # deliberate: stateless` with a detected PostgreSQL, and `frontend: N/A` with React, while saying existing values were kept. Now no detected value replaces it, and it keeps its own text: `database: None # deliberate: stateless` stays exactly as you wrote it, comment included. Scoring counts it as `slotignored` (the same decision, so the score is the same as if the file said `slotignored`); faf writes the word `slotignored` only into a slot that was empty. This is also true when the none is under the slot's other name (`stack.db` for `stack.database`), and for a slot that already says `slotignored` under either name. Outside the 33 slots, a none is kept exactly as written. `faf ai enhance` and `faf go` skip such a slot too: no AI suggestion and no interview answer replaces it.
...
- **An alias is never replaced.** With `stack: *base` (stack follows `defaults: &base`), `faf auto` filled a stack slot by replacing `*base` with a copy of the map: the alias, its comment and the link to `defaults` were gone. faf now leaves every alias as written. `faf auto` fills nothing under it and prints one line ("stack is an alias (*base) — faf left it as written"); `faf edit` of an alias key refuses with that line and exit 1; `faf go`, `faf ai`, `faf migrate`, `faf recover`, `faf conductor` and `faf sync --direction pull` print it when a change they carry falls on an alias. A change to the value an alias points to still lands, and the alias follows it.
...
- **A hand-written None / N/A / not applicable keeps its own text and scores as `slotignored`.** faf now keeps those words in the file as you wrote them, and `scoreFafYaml` and the exported `kernel.score` read them at a slot as `slotignored`. A file with such a slot scores a little higher than in 7.12, where the kernel counted it as an empty slot; it scores the same as round 2's `faf auto` output, which wrote `slotignored` in its place.
...
- `isExplicitNone(value)`, `EXPLICIT_NONE` and `SLOTIGNORED` are exported, so consumers treat a hand-written none the way faf does.
