...
function parseFafGitOutput(output: string): { currentScore: number; newScore: number; improvement: number; tier: string } | null {
  const currentMatch = output.match(/Current \(from repo\):\s+(\d+)%/);
  const newMatch = output.match(/New \(with FAF\):\s+(\d+)%\s+🏆?\s*(\w+)?/);
  const improvementMatch = output.match(/Improvement:\s+\+(\d+)\s+points/);
  ...
  return {
    currentScore: parseInt(currentMatch[1]),
    newScore: parseInt(newMatch[1]),
    improvement: improvementMatch ? parseInt(improvementMatch[1]) : 0,
    tier: newMatch[2] || 'Unknown'
  };
}
