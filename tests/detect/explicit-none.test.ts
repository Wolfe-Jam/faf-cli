 ...
 * one; when faf writes the slot it records `slotignored`, and the comment
 * stays on the line.
 */
...
describe('BRAKE: explicit none — never replaced by a detected value', () => {
  ...
  test('fillEmpties: at a slot, a hand-written none becomes slotignored — the detected value is not used', () => {
    ...
    expect(stackOf(out)).toEqual({ database: 'slotignored', frontend: 'slotignored', hosting: 'slotignored', cicd: 'slotignored', build: 'Vite' });
  });
  ...
  test('updateExistingFaf (faf auto): None / N/A / not applicable become slotignored, not PostgreSQL / React / a host', () => {
    const out = updateExistingFaf(repo(), {
      project: { name: 'web', type: 'frontend' },
      stack: { database: 'None', frontend: 'N/A', hosting: 'not applicable' },
    });
    expect(stackOf(out).database).toBe('slotignored');
    expect(stackOf(out).frontend).toBe('slotignored');
    expect(stackOf(out).hosting).toBe('slotignored');
  });
  ...
  test('a slot marked none (or slotignored) under its other name is not filled under this one', () => {
    ...
    expect(stackOf(out).framework).toBe('slotignored');
    ...
    expect(stackOf(both).db).toBe('slotignored');
  });

  test('the written file: the decision is recorded as slotignored and the comment stays on its line', () => {
    const dir = repo();
    ...
    expect(text).toContain('\n  database: slotignored # deliberate: stateless\n  frontend: slotignored\n  hosting: slotignored\n');
    const stack = stackOf(readFaf(path) as Record<string, unknown>);
    expect([stack.database, stack.frontend, stack.hosting]).toEqual(['slotignored', 'slotignored', 'slotignored']);
  });
